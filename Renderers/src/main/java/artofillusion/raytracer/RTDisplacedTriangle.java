...
/**
 * RTDisplacedTriangle represents a displacement mapped triangle to be raytraced. It is defined by
 * specifying a RenderingMesh, the index of the RenderingTriangle within the mesh, and a matrix
 * which gives the transformation from the mesh's local coordinates to world coordinates. To save
 * time and memory, the constructor is also passed two arrays which contain the vertices and normals
 * for the mesh, transformed into world coordinates.
 */
public class RTDisplacedTriangle extends RTObject {
    ...
    /**
     * This inner class holds many internal fields that are only used after the triangle is
     * initialized. Putting them in a separate object saves memory on triangles that never get hit
     * by a ray (and hence never get initialized).
     */
    ...
    public RTDisplacedTriangle(RenderingMesh mesh, int which, Mat4 fromLocal, Mat4 toLocal, double tol, double time) {
        ...
        int i = 0;
        if (trueNorm.dot(norm1) < 0.0) {
            i++;
        }
        if (trueNorm.dot(norm2) < 0.0) {
            i++;
        }
        if (trueNorm.dot(norm3) < 0.0) {
            i++;
        }
        if (i > 1) {
            trueNorm.scale(-1.0);
    ...
    /**
     * Determine whether this triangle is really displaced. If the displacement map is completely
     * flat over it, return false. Otherwise, return true.
     */
    ...
    /**
     * Part of the initialization is done lazily to save time and memory, since some triangles may
     * never be hit by any ray.
     */
    ...
    /**
     * Given the coordinates of a point along the ray, calculate the barycentric coordinates (u, v,
     * w) cooresponding to it. Also calculate the displacement h of that point on the triangle, and
     * return the height (the distance along the non-interpolated normal). If u, v, or w is outside
     * the range [0, 1], the return value is meaningless.
     */
    ...
    /**
     * Given the coordinates of a point along the ray, and a guess about the displacement h, find
     * the corresponding guesses about u, v, and w.
     */
    ...
    /**
     * Determine whether a ray intersects the bounding box. If so, return true and set mint and maxt
     * to the points where the ray enters and exits the box.
     */
    ...
    public static class DisplacedTriangleIntersection implements SurfaceIntersection {
         ...
         * numIntersections(), since that indicates that all intersections will be needed, not just
         * the first.
         */
