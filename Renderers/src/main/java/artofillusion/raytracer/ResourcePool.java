...
/**
 * A ResourcePool maintains a set of reusable objects that can be checked out when needed. This is
 * used to avoid unnecessary object creation in time critical routines. This class is not thread
 * safe, so a ResourcePool should not be shared between threads. Also, there is no way to return a
 * single object to the pool. Once you are done with all objects that have been taken from the pool,
 * call reset() to allow all of them to be reused.
 */
