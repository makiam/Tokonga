...
public class RaytracerRenderer implements Renderer, Runnable {
    ...
    /**
     * This routine is called when rendering is finished. It sets variables to null and runs a
     * garbage collection.
     */
    ...
    /**
     * This routine sends out a new ray, starting from the viewpoint and passing through pixel (i,
     * j). Number indicates which ray this is within the pixel, and is used for distribution ray
     * tracing. The light color is returned in color[0], and the transparency in transparency[0].
     */
     ...
     * @return the object with a material which the point is inside, or null if it is not inside any
     * material.
     */
    ...
    /**
     * This routine is called recursively to spawn new rays. It traces the ray, spawning still other
     * rays as necessary, and returns the total light incident on the ray origin from the specified
     * direction. The appropriate Ray object should be set up before calling this method, and the
     * light color is returned in the appropriate RGBColor object.
     *
     * @param workspace contains information for the thread currently being executed
     ...
     * @param prevMaterial the MaterialMapping the ray was passing through before entering
     * currentMaterial
     ...
     * @param transmitted true if this ray has only been transmitted (not reflected) since leaving
     * the eye
    ...
    /**
     * Find the direct lighting contribution to the surface color. The surface properties for the
     * given point should be in surfSpec[treeDepth], and the resulting color is returned in
     * color[treeDepth].
     *
     * @param workspace contains information for the thread currently being executed
    ...
    /**
     * Trace a ray to a light source, and determine which objects it intersects. If the ray is
     * completely blocked, such that no light from the light source reaches the ray origin, return
     * false. Otherwise, return true, and reduce the intensity of color[treeDepth] to give the
     * amount of light which reaches the ray origin. Arguments are:
     *
     * @param workspace contains information for the thread currently being executed
     ...
     * <p>
     * On exit, color[treeDepth] is set equal to the emitted color, and rayIntensity[treeDepth] is
     * reduced by the appropriate factor to account for the absorbed light.
     *
     * @param workspace contains information for the thread currently being executed
    ...
    /**
     * Find the light being scattered by a point in scattering material. The surface properties for
     * the given point should be in surfSpec[treeDepth-1], and the resulting color is returned in
     * color[treeDepth-1].
     *
     * @param workspace contains information for the thread currently being executed
     * @param treeDepth the current ray tree depth
    ...
    /**
     * Add a random displacement to a vector. The displacements are uniformly distributed over the
     * volume of a sphere whose radius is given by size. number is used for distributing the
     * displacements evenly.
     */
     ...
     * translucency effects. dir is a unit vector in the "ideal" reflected or refracted direction,
     * which on exit is overwritten with the new direction. norm is the local surface normal,
     * roughness determines how much the ray direction is altered, and number is used for
     * distributing rays evenly.
     */
    ...
    /**
     * Sort the list of MaterialIntersection objects by position along a shadow ray. This is done
     * with a simple insertion sort. Because the list tends to be very short, and is in close to the
     * correct order to begin with, this will generally be very fast.
     */
