...
/**
 * <p>
 * Raytracer implements a raytracing engine. It compares rays to objects in a scene to identify what
 * they hit.
 * </p>
 * <p>
 * To use it, first you must define the scene. Do this by calling {@link #addObject(ObjectInfo)} for
 * each object you want included in the scene. When all objects have been added, call
 * {@link #finishConstruction()} to analyze the scene and build data structures for use during
 * raytracing. There are several properties you can set that affect how the objects are interpreted.
 * For example, {@link #setSurfaceError(double) setSurfaceError()} determines how finely surfaces
 * should be triangulated, and {@link #setAdaptive(boolean) setAdaptive()} determines whether the
 * surface accuracy should vary based on the distance of the object from a camera.
 * </p>
 * <p>
 * While building the scene, it also records a list of {@link RTLight} objects representing light
 * sources in the scene. You can query the list by calling {@link #getLights()}, but otherwise it is
 * not used.
 * </p>
 * <p>
 * To trace a ray and see what it hits, call {@link #traceRay(Vec3, Vec3)}. There is also a second
 * version of the method that is more efficient, but requires more preparation by the caller before
 * it is invoked.
 * </p>
 * <p>
 * When you are all finished with the Raytracer, it is a good idea to call {@link #cleanup()} to set
 * internal pointers to null. This is not required, but may help the garbage collector to work more
 * efficiently.
 * </p>
 */
public class Raytracer {
    ...
    /**
     * When a ray is traced to determine what objects it intersects, a RayIntersection object is
     * used for returning the results. Typically it reports only the first object that was hit, but
     * when when two objects are almost exactly the same distance away, it reports both of them to
     * ensure that neither is missed.
     */
    public static class RayIntersection {
        ...
        /**
         * Get the details of the first intersection. If no object was hit, this will equal
         * {@link SurfaceIntersection#NO_INTERSECTION}.
         */
        ...
        /**
         * Get the details of the second intersection. In most cases, this will equal
         * {@link SurfaceIntersection#NO_INTERSECTION}.
         */
     ...
     *
     * @param scene the Scene which the objects that will be added with
     * {@link #addObject(ObjectInfo)} belong to
     * @param camera if {@link #isAdaptive()} is true (the default), this Camera is used as the
     * viewpoint for determining surface accuracy. Some objects may also vary their properties based
     * on how they are positioned relative to the camera.
     */
    ...
    /**
     * Get the error tolerance to use when triangulating objects. This is the maximum distance any
     * point on the triangulated surface may be from the true surface. The default value is 0.02.
     */
    ...
    /**
     * Set the error tolerance to use when triangulating objects. This is the maximum distance any
     * point on the triangulated surface may be from the true surface. Calling this method affects
     * all future calls to {@link #addObject(ObjectInfo) addObject()}, but does not affect objects
    ...
    /**
     * Get whether a decreased surface accuracy should be used for objects that are distant from the
     * camera. The default value is true.
     */
    ...
    /**
     * Set whether a decreased surface accuracy should be used for objects that are distant from the
     * camera. Calling this method affects all future calls to
     * {@link #addObject(ObjectInfo) addObject()}, but does not affect objects that have already
     * been added.
     */
    ...
    /**
     * Get the current time. This affects objects whose properties vary with time. The default value
     * is 0.
     */
    ...
    /**
     * Get the current time. This affects objects whose properties vary with time. Calling this
     * method affects all future calls to {@link #addObject(ObjectInfo) addObject()}, but does not
     * affect objects that have already been added.
     */
    ...
    /**
     * Get whether the raytracer should use the existing preview meshes for objects, instead of
     * constructing new meshes based on the requested surface accuracy. The default value is false.
     */
    ...
    /**
     * Set whether the raytracer should use the existing preview meshes for objects, instead of
     * constructing new meshes based on the requested surface accuracy. Calling this method affects
     * all future calls to {@link #addObject(ObjectInfo) addObject()}, but does not affect objects
     * that have already been added.
     */
    ...
    /**
     * Get whether the raytracer should use an alternate representation of triangles that uses less
     * memory but requires more computation to identify ray intersections. The default value is
     * false.
     */
    ...
    /**
     * Set whether the raytracer should use an alternate representation of triangles that uses less
     * memory but requires more computation to identify ray intersections. Calling this method
     * affects all future calls to {@link #addObject(ObjectInfo) addObject()}, but does not affect
     * objects that have already been added.
     */
    ...
    /**
     * Get whether RTLight objects should be configured to generate soft shadows. The default value
     * is false.
     */
    ...
    /**
     * Set whether RTLight objects should be configured to generate soft shadows. Calling this
     * method affects all future calls to {@link #addObject(ObjectInfo) addObject()}, but does not
     * affect objects that have already been added.
     */
    ...
    /**
     * This must be called after all objects have been added to the scene and before any calls to
     * {@link #traceRay(Vec3, Vec3) traceRay()}.
     */
    ...
    /**
     * Set fields to null. This may be called when you are done with the Raytracer and will not use
     * it any more. This is not required, but may help the garbage collector to work more
     * efficiently.
     */
    public void cleanup() {
        objectList = null;
    ...
    /**
     * Trace a ray and determine the first object it hits (or the first two objects, if they are
     * almost exactly the same distance away).
     *
    ...
    /**
     * Trace a ray and determine the first object it hits (or the first two objects, if they are
     * almost exactly the same distance away). This version of traceRay() is more efficient, but
     * requires more setup work by the caller.
     *
