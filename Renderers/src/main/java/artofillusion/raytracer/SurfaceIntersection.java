...
public interface SurfaceIntersection {
    ...
    /**
     * Same as intersectionProperties(), except only return the transparent color. This can save
     * time in cases where only the transparency is required, for example, when tracing shadow rays.
     * This also allows you to specify which intersection to get the transparency for.
     */
    public void intersectionTransparency(int n, RGBColor trans, double angle, double size, double time);

    /**
     * Get the normal of the "true" surface (without interpolation, bump mapping, etc.) at the nth
