...
/**
 * RTLight represents a light source in the scene to be raytraced. It is an abstract class, with
 * subclasses for particular types of lights.
 */
public abstract class RTLight {
    ...
    /**
     * This is called by the raytracer to generate a shadow ray from a point in the scene to the
     * light.
     *
     * @param origin the location from which to trace a ray
     * @param ray this should be configured to be a ray pointing from the specified origin to a
     * randomly chosen point inside the light
     * @param renderer the renderer being used to render an image
     * @param rayNumber a number which will be different for different rays, and can be used for
     * stratified sampling. If this is -1, that indicates that soft shadows should be disabled.
