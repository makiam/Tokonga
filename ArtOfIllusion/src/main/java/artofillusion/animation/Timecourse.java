...
/**
 * This class represents a quantity which changes as a function of time. It is defined by a series
 * of timepoints, with a value at each one. There is also a smoothness value for each timepoint,
 * which affects how it is interpolated.
 */
public class Timecourse {
    ...
    public Keyframe evaluate(double t, int method) {
        ...
        int minSubdivisions = (method == INTERPOLATING ? 5 : 9);
        ...
        while (true) {
            if (numSubdivisions >= minSubdivisions) {
                ...
                if (t1[i] - t1[i - 1] < 1.0 / 120.0 || !subdivideAdaptively) {
                    // Use linear interpolation to get the final value.

                    double fract = (t - t1[i - 1]) / (t1[i] - t1[i - 1]);
    ...
    /**
     * The following two methods are called by the evaluate() method. Given a value of time (t) and
     * a set of timepoints (v1, t1, s1), they return a new set of timepoints (v2, t2, s2) found by
     * subdividing the initial timepoints around t.
     */
