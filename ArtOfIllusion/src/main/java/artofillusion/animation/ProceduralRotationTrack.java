...
public class ProceduralRotationTrack extends Track implements ProcedureOwner {
    ...
    public void editProperties(ProcedureEditor editor) {
        ...
        ComponentsDialog dlg = new ComponentsDialog(editor.getParentFrame(), Translate.text("rotationTrackTitle"), new Widget[]{smoothChoice, modeChoice, jointChoice, coordsChoice, objSelector}, new String[]{Translate.text("paramSmoothingMethod"), Translate.text("trackMode"), Translate.text("applyTo"), Translate.text("CoordinateSystem"), ""});
        if (!dlg.clickedOk()) {
            return;
        }
