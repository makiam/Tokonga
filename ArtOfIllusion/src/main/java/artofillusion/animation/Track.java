...
/**
 * This is an abstract class representing an aspect of the scene which changes with time. Tracks are
 * typically defined either by a Timecourse or a Procedure.
 */
public abstract class Track {
    ...
    /**
     * Get the allowed range for graphable values. This returns a 2D array, where elements [n][0]
     * and [n][1] are the minimum and maximum allowed values, respectively, for the nth graphable
     * value.
     */
    ...
    /**
     * This method should modify whatever aspects of the scene are governed by this track, so that
     * they correspond to their values at the specified time.
     */
    ...
    /**
     * Set a keyframe at the specified time, based on the current state of the Scene, if and only if
     * the Scene does not match the current state of the track. Return the new keyframe, or null if
     * none was set.
     */
    ...
    /**
     * A null track is one which has no affect on the scene. This usually means that no keyframes
    ...
    /**
     * Get an array of any objects which this track depends on (and which therefore must be updated
     * before this track is applied).
     */
    ...
    /**
     * Delete all references to the specified object from this track. This is used when an object is
    ...
    /**
     * Update any references to objects this track depends on. Any reference to an object found as a
     * key in the map should be replaced with the corresponding object. This is used, for example,
     * when copying and pasting objects between scenes.
     */
