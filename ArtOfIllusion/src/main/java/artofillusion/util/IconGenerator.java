 ...
 *
 * IconGenerator supports a number of basic image operations such as overlay, blend, add, multiply,
 * etc, and can apply these to the pixels of one or more images.
 *
 * IconGenerator implements a basic macro processor, and the ability to execute a macro in source
 * (string) form, or to compile source and to execute the compiled version.
 *
 * IconGenerator supports a default syntax for macros, but the symbols mapped to each operation can
 ...
 * <ul>
 * <li>COMPOSIT semicolon (;) - at the end of each <i>instruction</i>, the result of the instruction
 * is composited over the current image.
 * <br><li>OVERLAY pipe (|) - the lhs is overlaid over an equal-sized rectangle of colour specified
 * by rhs
 * <br><li>ADD plus (+) - the rhs (an RGB colour) is added, component by component, to each pixel of
 * lhs
 * <br><li>SUBTRACT minus (-) - the rhs (an RGB colour) is subtracted, component by component, from
 * each pixel of lhs
 * <br><li>MULTIPLY asterisk (*) - each pixel of lhs is multiplied, component by component, by rhs
 * (an ARGB colour, or a list of scalar (floating point) values). If a component in RHS is not
 * speified, then the corresponding component in LHS is left unmodified.
 * <br><li>ASSIGN equals (=) - rhs (an ARGB colour) is assigned, component by component, to each
 * pixel of lhs. If any component of RHS is not specified, then the corresponding component in LHS
 * is left unmodified.
 * <br><li>FEATHER tilde (~) - lhs has feathering applied to selected pixels using the value(s) in
 * rhs to determine the type of feathering applied:
 ...
 * <br><li>BOUNDS square brackets ([]) - sets or modifies the current instruction by altering the
 * bounds (size) of the image (larger or smaller)
 * <br><li>POSITION at (@) - modifies the current instruction by altering the image position when it
 * is composited.
 * <br><li>RESOLVE parens ({}) - specifies a name as being a logical name in the namespace. A name
 * is resolved by searching the namespace for a matching string. Resolution is recursive, meaning
 * that if the result of the namespace lookup is a resolvable string, then that string is itself
 * looked up.
 ...
 * <br>egs: 0xff =&gt; alpha=1; 0x112233 =&gt; red=11, green=22, blue=33; 0xff112233 =&gt; alpha=ff,
...
public class IconGenerator {
    ...
    /**
     * copy the original image to a new image of the specified size (may be larger or smaller). The
     * content of the <i>orig</i> image is centered within the new image.
     *
     * @param orig the image to copy <i>from</i>
     * @param width the width of the new image
     * @param height the height of the new image.
     * @param scale the way to scale original when copying. scale == 0 means no scaling; scale &gt;
     * 0 means scale original to new dims * scale scale &lt; 0 means scale original abs(scale)
     * piselx smaller than new sims
     *
     * @return a newly allocated image of the specified ize containing the content of <i>orig</i>
     * centered within it.
     */
     ...
     * @param depth the "size" and "direction" of the bevel, which results in the specified apparent
     * "depth" or "height".
     *
    ...
    /**
     * draw a 3d bevelled edge on or in the image, with the edges being those of the specified clip
     * rectangle.
     *
     * @param image the image to modify
     * @param depth the size and "direction" of the bevelled edge. a positive value creates a
     * "popped-out" effect by making the top and left edges lighter and the bottom and right edges
     * darker. A negative value creates a "pressed-in" effect by doing the opposite. The absolute
     * value determines the apparent depth or height of the 3d effect by determining the width in
     * pixels of the bevelled edge. So 1 creates an edge 1 pixel wide, and 3 creates an edge 3
     * pixels wide.
     */
    ...
    /**
     * assign the colour components to each pixel in a rectangle within the image. If any component
     * value is negative, then that component is not modified.
     *
     * @param image the image to modify
     *
     ...
     * @param clip the region of <i>image</i> to modify. If this is <i>null</i>, then <i>overlay</i>
     * is centered on <i>image</i>.
     */
     ...
     * @param alpha the alpha value to use for the antialias strongest AA (antialias) pixels. 0.0
     * &lt; alpha &lt; 1.0 The Strongest AA pixels are those with the most adjoining opaque pixels
     * (currently 4 is the maximum).
     * @param attenuate the scaling factor to apply for increasingly weaker AA pixels. 0.0 &lt;
     * attenuate. Lighter AA pixels are those that have fewer adjoining opaque pixels (currently 3
     * is the minimum).
     *
    ...
    /**
     * The XSIZE and YSIZE parameters determine how far from the edge of the image to start
     * feathering, and the DIRection specifies whether to feather OUT (FEATHER_OUT_DIR) towards the
     * edges, or IN (FEATHER_IN_DIR) towards the centre.
     *
     * @param image the image to modify
     ...
     * @param dir specifies whether to feather OUT towards the edges, or IN towards the centre. see
     * {@link #FEATHER_OUT_DIR} and {@link #FEATHER_IN_DIR}
     *
     * @param clip defines the edges for feathering. If this is non-null, and specifies values other
     * than (0, 0, image.width, image.height), then the feathering is performed relative to the
     * edges of this rectangle.
     *
     * This is useful if the feathered image is going to be overlaid on some other image using an
     * identical clip region.
     */
