...
/**
 * TriMeshSimplifier defines methods for simplifying a TriangleMesh. The algorithm is based on
 * <p>
 * R. Ronfard and J. Rossignac, "Full-range Approximation of Triangulated Polyhedra." Computer
 * Graphics Forum, vol. 15, no. 3, 1996.
 * <p>
 * I have made the following modifications to their algorithm:
 * <p>
 * 1. When calculating the cost for an edge collapse, they arbitrarily choose which direction to
 * collapse it in. This results in meshes not being simplified as far as they could be. Instead, I
 * always test both collapse directions to see which one has the lower cost.
 * <p>
 * 2. After collapsing vertex v1 to v2, they state that it is only necessary to recalculate costs
 * for edges which have been modified, that is, those edges in the star of v1 or v2. This is
 * incorrect. It is also necessary to recalculate the cost for any edge which shares a vertex with
 * an edge in v1's star.
 * <p>
 * 3. They define the local tesselation error for a face as being proportional to its angle of
 * rotation. I replace the angle by 1-cos(angle). This has the same effect, and saves the need to
 * evalate an acos.
 * <p>
 * 4. Rather than always simplifying the entire mesh, I allow the user to select only a subset of
 * the mesh to be simplified.
 * <p>
 * 5. Their algorithm only works for closed meshes (meshes without a boundary). I have modified it
 * to also work for open meshes.
 * <p>
