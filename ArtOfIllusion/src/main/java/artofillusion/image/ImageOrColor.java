...
/**
 * ImageOrColor represents a color which can potentially vary with position. It provides a Widget
 * with which the user can edit it. They can choose either a single color, or an ImageMap which is
 * then multiplied by a user specified color.
 */
public class ImageOrColor {
    ...
    /**
     * Given a texture coordinate (x and y each between 0 and 1), return the average color of a
     * region of width (xsize, ysize) centered at that location. wrapx and wrapy specify whether,
     * for purposes of interpolation, the image should be treated as wrapping around so that
