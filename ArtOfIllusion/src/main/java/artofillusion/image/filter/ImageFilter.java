...
/**
 * This class defines an object which can filter rendered images. It is an abstract class, whose
 * subclasses implement specific types of filters. An ImageFilter may present a user interface for
 * configuring filtering options, and also may have a list of keyframeable parameters.
 */
public abstract class ImageFilter {
    ...
    /**
     * Get a list of all the image components required by this filter. This should be a sum of the
     * constants defined in ComplexImage. The renderer will attempt to provide all requested
     * components, but some renderers may not support all components. The filter should therefore be
     * prepared for the possibility that some components may be null (aside from the basic red,
     * green, blue, and alpha components, which are always available).
     */
