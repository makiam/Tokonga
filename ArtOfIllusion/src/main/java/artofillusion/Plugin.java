 ...
 * Changes copyright (C) 2021-2026 by Maksim Khramov
...
public interface Plugin {
    ...
    default void processMessage(int message, Object... args) {
        switch (message) {
            case Plugin.APPLICATION_STARTING -> onApplicationStarting();
            case Plugin.APPLICATION_STOPPING -> onApplicationStopping();
            case Plugin.SCENE_WINDOW_CREATED -> onSceneWindowCreated((LayoutWindow) args[0]);
            case Plugin.SCENE_WINDOW_CLOSING -> onSceneWindowClosing((LayoutWindow) args[0]);
            case Plugin.OBJECT_WINDOW_CREATED -> onObjectWindowCreated((ObjectEditorWindow) args[0]);
