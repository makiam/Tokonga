...
public class LayoutWindow extends BFrame implements EditingWindow, PopupMenuManager, PropertyChangeListener {
  ...
  private final KeyEventPostProcessor keyEventHandler;
    ...
    /**
     * Rebuild the TreeList of objects, attempting as much as possible to preserve its current
     * state.
     */
    ...
    /**
     * Rebuild the list of tool scripts in the Tools menu. This should be called whenever a script
     * has been added to or deleted from the Scripts/Tools directory on disk.
     */
    ...
    /**
     * This method is called to close the window. If the Scene has been modified, it first gives the
     * user a chance to save the Scene, or to cancel. If the user cancels it, the method returns
     * false. Otherwise, it closes the window and returns true.
     */
    @Override
    ...
    /**
     * When a tool gets selected in the tool palette, notify the UI. It may be possible, that some
     * options need to be disabled/changed etc.
     */
    ...
    /**
     * Get the Frame corresponding to this window. (Because LayoutWindow is a Frame, it simply
     * returns itself.)
     */
    @Override
    ...
    public void updateMenus() {
        ...
        if (numSelObjects == 0) {
            for (BMenuItem item : objectMenuItem) {
    ...
    /**
     * Remove an object from the scene. If undo is not null, appropriate commands will be added to
     ...
     *
     * @param view the ViewerCanvas which should become the currently focused view. If this is not
     * one of the views belonging to this window, this method does nothing.
     */
    ...
    private void showCoordinateAxisAction(CommandEvent event) {
        ...
        for (SceneViewer cv : theView) {
            cv.setShowAxes(currentState);
        }
    ...
    /**
     * Execute the tool script contained in a file, passing a reference to this window in its
     * "window" variable.
     */
