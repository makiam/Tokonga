...
public class ScrollViewTool {
    ...
    protected void mouseScrolled(MouseScrolledEvent e, ViewerCanvas v) {
        ...
        Point mousePoint = view.mousePoint = e.getPoint();
    ...
    private void scrollMoveTravel(MouseScrolledEvent e) {
        ...
        if (navigationMode == ViewerCanvas.NAVIGATE_TRAVEL_SPACE) {
            ...
            double scrollRadius = Math.sqrt(rx * rx + ry * ry);
            if (scrollRadius < 0.1 * d) {
                scrollRadius = 0.1 * d;
            }
            if (scrollRadius > 0.4 * d) {
                scrollRadius = 0.4 * d;
            }
            double scrollBlend = (scrollRadius - 0.1 * d) / (0.3 * d);
            ...
            if (Math.abs(scrollX) < 0.1 * d) {
                scrollX = 0.1 * d * Math.signum(scrollX);
            }
            if (Math.abs(scrollX) > 0.4 * d) {
                scrollX = 0.4 * d * Math.signum(scrollX);
            }
            double scrollBlendX = (Math.abs(scrollX) - 0.1 * d) / (0.3 * d);

            double scrollY = scrollPoint.y - cy;
            if (Math.abs(scrollY) < 0.1 * d) {
                scrollY = 0.1 * (double) d * Math.signum(scrollY);
            }
            if (Math.abs(scrollY) > 0.4 * d) {
