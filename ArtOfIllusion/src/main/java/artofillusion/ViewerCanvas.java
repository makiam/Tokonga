...
public abstract class ViewerCanvas extends CustomWidget {
    ...
    protected final Timer mouseMoveTimer = new Timer(500, new ActionListener() {
  ...
  * The 'blend' is the balance of the color in range 0.0 - 1.0 from color0 to color1
    ...
    private Vec3 viewingPoint3D;
        ...
        public void draw(ViewerCanvas viewToDrawOn) {
            ...
            Color shapeColor = new Color((int) (planeColor.getRed() * (1 - viewCos2) + backgroundColor.getRed() * viewCos2),
            ...
            // used in conversions 3D to screen 2D
            Vec2 vector2D;
    ...
    private class CoordinateAxes {
        ...
        void draw() {
            ...
            Mat4 rotate = Mat4.viewTransform(origin, cameraCoords.getZDirection(), cameraCoords.getUpDirection().times(-1));
            graphics.clearRect(0, 0, imageSize - 1, imageSize - 1);
            graphics.setColor(axisColor);
            int axis;
            for (axis = 0; axis < 3; axis++) {
                Vec2 axis2D = rotate.timesXY(axis3D[axis]).times(axisLength);
                Vec2 label2D = rotate.timesXY(axis3D[axis]).times(axisLength + labelD * 2);
                ...
                if (label2D.minus(axis2D).length() > (double) labelD / 4.0) {
                    Point labelPos = point(label2D, imageCenter);
                    int i;
                    for (i = 0; i < labelPoint[axis].length; i++) {
                        int[] dc = drawingCoords(labelPoint[axis][i], labelPos);
