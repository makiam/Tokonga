...
/**
 * ViewerCanvas is the abstract superclass of all components which display objects, and allow the
 * user to manipulate them with EditingTools.
 */
public abstract class ViewerCanvas extends CustomWidget {
    ...
    /**
     * For object editors this tool is set in the abstract ObjectEditorWindow, whereas the alt- and
     * meta tools are set in each sub(-sub-sub) class of it. This way it gets inherited to plugin
     * tools too (like PME).
     */
     ...
     * <p>
     *
	  When the mode changes the view scale and camera distance from drawing plane are recalculated
     * so that the perceived scale on the drawing plane does not change.<p>
     *
	  For animated perspective changes the view has to be in perspective during the animation and
     * the perspective-parameter is turned false at finishAnimation() if needed. The
     * perspectiveSwitch- parameter tells the users last perspehtive selection during animation.
     */
    ...
    /**
     * This is needed when animated perspective change parallel to perspective begins. The scale and
     * perspective parameters can not be accessed directly form the animation engine and they take
     * immediade effect on the camera.
     */
    ...
    /**
     * ViewAnimation calls this when the animation is finished, so the orientation menu will be up
     * to date and the perspective is set to it's value without launched launcing a new animation
     * sequence.
     */
    ...
    /**
     * Get the location around which the view should be rotated. This may be null, in which case the
     * value returned by {@link #getDefaultRotationCenter()} will be used instead.
     */
    ...
    /**
     * Set the location around which the view should be rotated. This may be null, in which case the
     * value returned by {@link #getDefaultRotationCenter()} will be used instead.
     */
     ...
     * It should be made sure that the rotation center is always known and never null, but if null
     * is returned this recreates it and returns the new value
     */
    ...
    /**
     * Set the distance from drawing plane to view camera.
     * <p>
     *
	Depending on the view action this may be used to recalculate the positon of the camera or the
     * rotationCenter or it may be recalculated from the rotationCenter and camera position.
     */
    ...
    /**
     * Set navigation mode      *
     * @param nextNavigation may be one of NAVIGATE_MODEL_SPACE = 0; NAVIGATE_MODEL_LANDSCAPE = 1;
     * NAVIGATE_TRAVEL_SPACE = 2; NAVIGATE_TRAVEL_LANDSCAPE = 3;
     *
     * Setting the value higher than 3 will have MoveViewTool and RotateViewTool ignore mouse
     * commands. This may be helpful for plug-in added navigation modes.
     *
    ...
    protected void showPopupIfNeeded(WidgetMouseEvent ev) {
        if (popupManager != null && ev instanceof MouseClickedEvent && ev.getButton() == WidgetMouseEvent.BUTTON3 && ev.getClickCount() == 1) //if (popupManager != null && ev.isMetaDown() && ev.getClickCount() == 1)
        {
     ...
     *
      Adjust the camera position and magnification so that the specified box fills the view. This
     * has no effect if there is a camera bound to this view.
     */
    @Deprecated
     ...
     *
	@param selection If set to true, fits to selected vertices else fit to the entire mesh.
     */
    ...
    /**
     * Turn the view to match the closest main coordinate directions, which maybe positive or
     * negative.
     */
    public void alignWithClosestAxis() {
        ...
        if (theCamera == null) {
            if (zNew.z == -1 && upNew.y == 1) {
                newOrientation = 0; // Front
            }
            if (zNew.z == 1 && upNew.y == 1) {
                newOrientation = 1; // Back
            }
            if (zNew.x == 1 && upNew.y == 1) {
                newOrientation = 2; // Left
            }
            if (zNew.x == -1 && upNew.y == 1) {
                newOrientation = 3; // Right
            }
            if (zNew.y == -1 && upNew.z == -1) {
                newOrientation = 4; // Top
            }
            if (zNew.y == 1 && upNew.z == 1) {
                newOrientation = 5; // Bottom
            }
        } else {
    ...
    private Vec3 findNextAxis(Vec3 dir, Vec3 fixed) {
        ...
        if (Math.abs(dir.x) != maxD && Math.abs(dir.x) != minD) {
            nextDir = new Vec3(dir.x, 0, 0);
        } else if (Math.abs(dir.y) != maxD && Math.abs(dir.y) != minD) {
            nextDir = new Vec3(0, dir.y, 0);
        } else {
            nextDir = new Vec3(0, 0, dir.z);
        }
        nextDir.normalize();
        ...
        if (nextDir.equals(fixed) || nextDir.equals(fixed.times(-1))) {
            if (Math.abs(dir.z) != maxD && Math.abs(dir.z) != minD) {
                nextDir = new Vec3(0, 0, dir.z); // already z ?
            } else if (Math.abs(dir.y) != maxD && Math.abs(dir.y) != minD) {
                nextDir = new Vec3(0, 0, dir.y); // already checked
            } else {
                nextDir = new Vec3(0, 0, dir.x); // this would be definitely different.
            }
        }
        nextDir.normalize();
    ...
    /**
     * This should be called by the CanvasDrawer just before rendering an image. It sets up the
     * camera correctly.
     */
    ...
    /**
     * Estimate the range of depth values that the camera will need to render. This need not be
     * exact, but should err on the side of returning bounds that are slightly too large.
     *
    ...
    /**
     * Subclasses should override this to draw the contents of the canvas, but should begin by
     * calling super.updateImage() to display the grid.
     */
    ...
    /**
     * Adjust the coordinates of a mouse event to move it to the nearest grid location.
     */
    ...
    /**
     * Launch an orientation change procedure to turn the orientation into any of the presets shown
    ...
    /**
     * Center the view to a point in the model space. No need to override by subclasses. Local
     * coordinates are handled by the ClickedPointFinder, 'finder'.
     */
     ...
     * This method should only ever be called from the event dispatch thread.
     */
    ...
    /**
     * Draw a horizontal line across the rendered image. The parameters are the y coordinate of the
     * line and the line color.
     */
    ...
    /**
     * Draw a vertical line across the rendered image. The parameters are the x coordinate of the
     * line and the line color.
     */
    ...
    /**
     * Render an image onto the canvas. The image is drawn as a planar rectangle in 3D space, which
     * you specify by its four corners in clockwise order, starting from the top left.
     *
    ...
    /**
     * Draw a line on the screen as segment of a radius from a center point. Angle ar radii and r0
     * and r1 as screen (pixel) coordinate scale.
     */
    ...
    /**
     * FrustumShape is the visulization of the 'drawinplane' where mouse actions happening and the
     * position of view camera. The ViewerCanvas parameter 'distToPlane' defines the distance
     * between the drawing palne theCamera.
     */
    ...
    private Vec3 viewingPoint3D;
         ...
         *
      The drawing has to happen pixel by pixel because the canvasdrawers handle 'drawLine()'
         * with a different logic.<p>
         *
