...
/**
 * A TextureMapping describes the mapping of a Texture's texture coordinates to points on the
 * surface of an object. It operates in two ways. First, it generates RenderingTriangles which
 * contain the necessary information to be rendered. Second, since some renderers can directly
 * render certain objects without ever breaking them into triangles, it must be able to directly
 * calculate the material properties for a point in space.
 * <p>
 * This is an abstract class. Its subclasses describe various types of mappings which are
 * appropriate for various types of objects and materials.
 */
public abstract class TextureMapping {
    ...
    /**
     * Every subclass of TextureMapping must define a constructor which takes a Texture and an
     * Object3D as its arguments:
     * <p>
     ...
     * <p>
     * which returns true if the mapping can be used with the specified object and Texture. Finally,
     * every subclass must include a constructor with the signature
     * <p>
     * public MappingSubclass(DataInputStream in, Object3D obj, Texture texture) throws IOException,
     * InvalidObjectException
     * <p>
     * which reconstructs the mapping by reading its data from an input stream. The following method
    ...
    /**
     * Get the name of this type of mapping. Subclasses should override this method to return an
     * appropriate name.
     */
    ...
    /**
     * Given the vertices to be mapped and their normal vectors, generate a RenderingTriangle. Most
     * subclasses will override this method. However, some mappings which require more information
     * than just the vertex coordinates may instead define new methods which replace this one.
     */
     ...
     * If necessary, subclasses can override this to take appropriate action.
     */
    public void setParameters(RenderingTriangle tri, double p1[], double p2[], double p3[], RenderingMesh mesh) {
    }

    /**
     * Given a point on the surface for which this mapping is being used, find the corresponding
     * surface properties. The properties should be averaged over a region around the point.
     *
    ...
    /**
     * Same as above, except only return the transparent color. This can save time in cases where
     * only the transparency is required, for example, when tracing shadow rays.
     */
    public abstract void getTransparency(Vec3 pos, RGBColor trans, double angle, double size, double t, double param[]);

    /**
     * Given a point on the surface, evaluate the displacement function. All parameters are the same
     * as for getTextureSpec().
     */
    ...
    /**
     * Create a new TextureMapping which is identical to this one, but for a different object and
     * texture.
     */
    ...
    /**
     * Get the list of texture parameters associated with this mapping and its texture. Subclasses
     * that define their own parameters should override this to add them to the list.
     */
    ...
    /**
     * This method should return a Widget in which the user can edit the mapping. The parameters are
     * the object whose mapping is being edited, and a MaterialPreviewer which should be rendered
     * whenever one of the mapping's parameters changes.
     */
