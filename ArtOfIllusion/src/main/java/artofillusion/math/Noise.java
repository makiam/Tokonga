...
/**
 * This class provides an interface for accessing a three dimensional noise function. By default it
 * uses the {@link SimplexNoise} class to calculate the function, but it is possible to specify an
 * alternate noise generator. This allows plugins to replace the noise function used throughout the
 * application, for example to provide a faster implementation using native code.
 * <p>
 * Regardless of the implementation, this should be a function which varies smoothly between -1.0
 * and 1.0, with all of the variation happening on a length scale of about 1. Typically, you will
 * want to add several octaves of this function together to create a fractal noise function. Methods
 * are also provided for calculating the gradient of the noise function, and a vector valued noise
 * function.
 */
public class Noise {
