...
/**
 * The TriangleMesh class represents an arbitrary surface defined by a mesh of triangular faces.
 * Depending on the selected smoothing method, the surface may simply consist of the triangular
 * faces, or it may be a smooth subdivision surface which either interpolates or approximates the
 * vertices of the control mesh.
 */
public class TriangleMesh extends Object3D implements FacetedMesh {

    /**
     * A vertex specifies a position vector, the number of edges which share the vertex, and the
     * "first" edge. If the vertex is in the interior of the mesh, any edge can be the first one. If
     * it is on the boundary, then the first edge must be one of the two boundary edges. A vertex
     * also has a "smoothness" parameter associated with it.
     */
    public class Vertex extends MeshVertex {
        ...
        /**
         * This method tells whether the list of edges returned by getEdges() are ordered clockwise
    ...
    /**
     * An edge is defined by the two vertices which it connects, and the two faces it is adjacent
     * to. For a boundary edge, f2 must equal -1. An edge also has a "smoothness" parameter.
     */
    public class Edge {
    ...
    /**
     * A face is defined by its three vertices and three edges. The vertices must be arranged in
     * counter-clockwise order, when viewed from the outside. Edges 1, 2, and 3 connect vertices 1
     * and 2, 2 and 3, and 3 and 1 respectively.
     */
    public class Face {
        ...
        /**
         * Given another face, return the index of the edge it shares with this one, or -1 if they
    ...
    /**
     * The constructor takes three arguments. v[] is an array containing the vertices. faces[][] is
     * an N by 3 array containing the indices of the vertices which define each face. The vertices
     ...
     * this condition, and the results are undefined. norm[] contains the normal vector at each
     * vertex. If any element of norm[] is null, flat shading will be used around that vertex.
     */
    ...
    /**
     * This method rebuilds the mesh based on new lists of vertices and faces. The smoothness values
     * for all edges are lost in the process.
     */
    ...
    /**
     * Calculate a set of array representing the boundaries of this mesh. There is one array for
     * each distinct boundary, containing the indices of the edges which form that boundary.
     */
    ...
    /**
     * Given a pair of vertices and a new vertex that is to be created between them, find the IK
     * binding parameters for the new vertex.
     */
    ...
    /**
     * Subdivide all or part of the mesh using the mesh's defined smoothing method (linear,
     * approximating, or interpolating).
     *
     * @param mesh the mesh to subdivide
     * @param splitEdge a flag for each edge, specifying which ones should be split. If this is
     * null, every edge will be split.
    ...
    /**
     * This method subdivides each selected edge once, placing a new vertex in the midpoint of the
     * edge, and returns the subdivided mesh.
     *
     * @param mesh the mesh to subdivide
     ...
     * TriangleMesh which approximates the limit surface to within the specified tolerance. The
     * subdivision coefficients are taken (with a few minor changes) from Hoppe et al. "Piecewise
     * Smooth Surface Reconstruction." SIGGRAPH Proceedings, 1994, p. 295. The algorithm for
     * creating semi-smooth points and creases is original.
     *
     * @param mesh the mesh to subdivide
     * @param refineEdge a flag for each edge, specifying which ones should be split. If this is
     * null, every edge will be split.
     * @param tol the error tolerance. Edges will be repeatedly subdivided until every point on the
     * subdivided mesh is within this distance of the limit surface. If this is equal to
     * Double.MAX_VALUE, each edge will be subdivided exactly once.
    ...
    /**
     * This method subdivides the mesh using interpolating (modified Butterfly) subdivision, and
     * returns a new TriangleMesh which approximates the limit surface to within the specified
     * tolerance. The subdivision coefficients are taken from Zorin et al. "Interpolating
     * Subdivision for Meshes with Arbitrary Topology." 1996. The algorithm for creating semi-smooth
     * points and creases is original.
     *
     * @param mesh the mesh to subdivide
     * @param refineEdge a flag for each edge, specifying which ones should be split. If this is
     * null, every edge will be split.
    ...
    /**
     * This method is used for Butterfly subdivision. Given a face and an edge, it finds the other
     * face which is across the edge from the specified one, finds the vertex of that face which is
     * opposite the specified edge, and returns its position in pos. The position of this "opposite
     * vertex" can be calculated two different ways. For smooth edges, it is the actual position of
     * the vertex. For boundary or crease edges, it is a "virtual vertex" created by mirroring the
     * specified face across the specified edge. The relative weights of these two are determined by
     * smoothWeight.
     */
    ...
    /**
     * This method is used for Butterfly subdivision. Given the number of edges intersecting an
     * extraordinary vertex, it returns an array containing the subdivision coefficients to use for
     * that vertex.
     */
    ...
    /**
     * This method is called by the various subdivideXXX() methods to do the actual subdivision. The
     * vertex, edge, and face arguments describe the mesh to be subdivided. newvert contains the
     * vertices of the new mesh. newedge and newface are empty arrays of the correct length, into
     * which the new faces and edges will be placed. mesh is the TriangleMesh which the new edges
     * and faces should belong to. split is an array specifying which edges of the old mesh should
     * be split.
     */
    ...
    /**
     * Create a new triangle mesh by subdividing this one until no edge is longer than the specified
     * tolerance.
     */
    ...
    /**
     * Create a new triangle mesh by applying the displacement map of the texture assigned to this
     * object.
     */
    ...
    /**
     * Return a new mesh which is an "optimized" version of the input mesh. This is done by
     * rearranging edges to eliminate very small angles, or vertices where many edges come together.
     * The resulting mesh will generally produce a better looking surface after smoothing is applied
     * to it.
     */
    ...
    /**
     * The following two methods are used for reading and writing files. The first is a constructor
     * which reads the necessary data from an input stream. The other writes the object's
    ...
    public void writeToFile(DataOutputStream out, Scene theScene) throws IOException {
        ...
        for (Vertex item : vertex) {
            item.r.writeToFile(out);
            out.writeFloat(item.smoothness);
            out.writeInt(item.ikJoint);
            out.writeDouble(item.ikWeight);
        }
        out.writeInt(edge.length);
        for (Edge item : edge) {
            out.writeInt(item.v1);
            out.writeInt(item.v2);
            out.writeInt(item.f1);
            out.writeInt(item.f2);
            out.writeFloat(item.smoothness);
        }
        ...
        for (Face item : face) {
            out.writeInt(item.v1);
            out.writeInt(item.v2);
            out.writeInt(item.v3);
            out.writeInt(item.e1);
            out.writeInt(item.e2);
            out.writeInt(item.e3);
        }
    ...
    /**
     * TriangleMeshes cannot be keyframed directly, since any change to mesh topology would cause
     * all keyframes to become invalid. Return an actor for this mesh.
     */
    @Override
    ...
    public static class TriangleMeshKeyframe extends MeshGesture {
         ...
         * or three others. These methods should never be called, since TriangleMeshes can only be
        ...
        public TriangleMeshKeyframe(DataInputStream in, Object parent) throws IOException, InvalidObjectException {
            ...
            for (int i = 0; i < numVert; i++) {
                ...
                if (version == 0) {
                    for (ParameterValue paramValue1 : paramValue) {
                        ((VertexParameterValue) paramValue1).getValue()[i] = in.readDouble();
                    }
                }
            }
            if (version > 0) {
