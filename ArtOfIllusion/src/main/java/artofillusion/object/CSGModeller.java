...
/**
 * This class is used by CSGObject. It performs the actual work of applying boolean operators to
 * create meshes based on the original objects. This is based on the algorithm described in
 * <p>
 * D. H. Laidlaw, W. B. Trumbore, and J. F. Hughes. "Constructive Solid Geometry for Polyhedral
 * Objects." SIGGRAPH Proceedings, 1986, p.161.
 * <p>
 * with some simplifications due to the fact that we only permit triangles, rather than arbitrary
 * convex polygons.
 */
public class CSGModeller {
    ...
    /**
     * Split a face of one of the component objects. As a result, new vertices can be added to vert
     * and new faces to face. Also, vertex type can be set to BOUNDARY. Parameters: vert: the full
     * list of vertices of the object face: the full list of faces of the object which: the index of
     * the face to split into the face vector intersectVert[]: the indexes of the 2 vertices (if
     * typeA[] = VERTEX) or of the two edges (if typeA[] = EDGE) between which the span runs
     * distA[]: the distances of the intersections of the "line" with the face being split, measured
     * from the root distB[]: the distances of the intersections of the "line" with the intersecting
     * face of the second object, measured from the root typeA[]: the type of the intersections:
     * VERTEX if the intersection is a vertex of the face being split; EDGE if the intersection
     * stays on an edge spanTypeA: the type of the "span" that is the result of the intersection of
     * the two faces: VERTEX in case the span is a single point, coincident with a vertex EDGE if
     * the span runs along an edge of the face POINT_ON_EDGE if the span is a single point, on an
     * edge FACE in all other cases line: a unity vector that, together with root, describes the
     * "line" of the intersection between the two faces root: where the "line" starts
     */
...
//          continue;
    ...
    /**
     * Determine whether a ray intersects the bounding box. Return the distance along the ray at
     * which it enters the box, or Double.MAX_VALUE if it does not intersect.
     */
    ...
    /**
     * Determine whether a ray intersects a triangle. Return the distance along the ray at which it
     * enters the triangle, or Double.MAX_VALUE if it does not intersect. If the ray lies in the
     * plane of the triangle, or passes through an edge of it, return -Double.MAX_VALUE.
     */
