...
/**
 * SceneCamera is a type of Object3D. It represents a camera which the user can position within a
 * scene. It should not be confused with the Camera class.
 */
public class SceneCamera extends Object3D {
    ...
    public void applyImageFilters(ComplexImage image, Scene scene, CoordinateSystem coords) {
        for (ImageFilter filter: filters) {
            filter.filterImage(image, scene, this, coords);
        }
        image.rebuildImage();
    }
    ...
    /**
     * Compute a ray from the camera location through a point in its field of, represented in the
     ...
     * @param dof1 this is used for simulating depth of field. dof1 and dof2 are independent values
     * uniformly distributed between 0 and 1. Together, they select the point on the camera which
     * should serve as the ray's origin.
    ...
    /**
     * This is a utility method which synchronously renders an image of the scene from the viewpoint
     * of this camera.
     */
        ...
        if (version == 0) {
            ...
            try {
                for (int i = 0; i < filters.length; i++) {
                    ...
                    filters[i] = (ImageFilter) cls.getDeclaredConstructor().newInstance();
    ...
    public void writeToFile(DataOutputStream out, Scene theScene) throws IOException {
        ...
        for (ImageFilter filter : filters) {
            ...
            filter.writeToStream(out, theScene);
        }
    }
