...
public abstract class Object3D {
    ...
    /**
     * Resize the object. This should be interpreted such that, if setSize() is followed by a call
     * to getBounds(), the dimensions of the BoundingBox will exactly match the dimensions specified
     * in setSize().
     */
    public abstract void setSize(double xsize, double ysize, double zsize);

    /**
     * Tells whether the object is closed. For curves, this means it has no endpoints. For surface,
     * it means the surface has no boundary.
     */
    ...
    /**
     * Tells whether the object can be converted to a TriangleMesh. It should return one of the
     * following values:
     *
     * CANT_CONVERT: The object cannot be converted to a TriangleMesh. EXACTLY: The object can be
     * represented exactly by a TriangleMesh. APPROXIMATELY: The object can be converted to a
     * TriangleMesh. However, the resulting mesh will not be exactly the same shape as the original
     * object.
     *
     ...
     * canConvertToTriangleMesh() returned APPROXIMATELY, this method should return a TriangleMesh
     * which reproduces the object to within the specified tolerance. That is, no point on the mesh
     * should be further than tol from the corresponding point on the original surface. If
     * canConvertToTriangleMesh() returned EXACTLY, then tol should be ignored. If
     * canConvertToTriangleMesh() return CANT_CONVERT, this method returns null.
     */
    ...
    /**
     * This will be called whenever this object is moved, or the time changes. Most objects will do
     * nothing here, and do not need to override this. It is available for those cases where an
     * object's internal properties depend explicitly on time or on the object's position within the
     * scene.
     */
    ...
    /**
     * Edit an object which represents a gesture for an Actor object. realObject specifies the
     * object in the scene which this is a gesture for.
     */
    ...
    /**
     * This method tells whether textures can be assigned to the object. Objects for which it makes
     * no sense to assign a texture (curves, lights, etc.) should override this method to return
     * false.
     */
    ...
    /**
     * This method tells whether materials can be assigned to the object. The default implementation
     * will give the correct result for most objects, but subclasses can override this if necessary.
     */
    ...
    /**
     * Set the Material and MaterialMapping for this object. Pass null for both arguments to specify
     * that the object does not have a Material.
     */
    ...
    /**
     * Copy all texture and material information from another object to this one. This method is
     * intended to be called by subclasses' implementations of duplicate() and copyObject().
     */
    ...
    /**
     * Objects which can be rendered as part of a scene should override this method to return a
     * RenderingMesh which describes the appearance of the object. All points on the RenderingMesh
     * should be within a distance tol of the true surface. The interactive flag tells whether the
     * resulting Mesh will be rendered in interactive mode. When interactive is set to true, the
     * RenderingMesh should be cached for future use, so that it may be rendered repeatedly without
     * needing to be regenerated.
     * <p>
     * The ObjectInfo contains additional information which may affect how the object is rendered,
     * such as it location in the scene, texture parameters, etc.
     * <p>
     * Objects which cannot be rendered directly (lights, cameras, curves, etc.) do not need to
    ...
    /**
     * Every object should override this method to return a WireframeMesh. This will be used for
     * drawing the object in wireframe mode, and also for drawing "nonrenderable" objects in other
     * rendering modes.
     */
    ...
    /**
     * The following method writes the object's data to an output stream. Subclasses should override
     * this method, but also call super.writeToFile() to save information about materials, etc. In
     * addition to this method, every Object3D must include a constructor with the signature
     *
     * public Classname(DataInputStream in, Scene theScene) throws IOException,
     * InvalidObjectException
     *
     * which reconstructs the object by reading its data from an input stream. This constructor,
     * similarly, should call the overridden constructor to read information about materials, etc.
     */
    ...
    /**
     * This will be called whenever a new pose track is created for this object. It allows the
     * object to configure the track by setting its graphable values, subtracks, etc.
     */
    ...
    /**
     * Get a version of this object to which a pose track can be attached. For most objects, this
     * simply returns itself. Some objects, however, need to be converted to Actors to have pose
     * tracks. If this method returns null, no pose track will be attached.
     */
