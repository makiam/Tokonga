...
/**
 * The SplineMesh class represents a parametric surface defined as a tensor product of spline
 * curves. Depending on the selected smoothing method, the surface may either interpolate or
 * approximate the vertices of the control mesh.
 */
public class SplineMesh extends Object3D implements Mesh {
    ...
    /**
     * v is an array containing the points of the control mesh, with the first index corresponding
     * to the U direction, and the second to the V direction. The two smoothness arrays give the
     * smoothness values along the U and V directions respectively.
     */
     ...
     * subdivision along the second dimension. It returns an array containing three elements: the
     * subdivided matrix, the new list of smoothness values, and the new list of parameter values.
     */
    ...
    /**
     * The following two routines are used by subdivideMesh to calculate new point positions for
     * interpolating and approximating subdivision. v is an array of vertices, s is the array of
     * smoothness values for them, and i, j, k, and m are the indices of the points from which the
    ...
    /**
     * If necessary, reorder the points in the mesh so that, when converted to a triangle mesh for
     * rendering, the normals will be properly oriented.
     */
    ...
    /**
     * SplineMeshes cannot be keyframed directly, since any change to mesh topology would cause all
    ...
    public static class SplineMeshKeyframe extends MeshGesture {
         ...
         * or three others. These methods should never be called, since SplineMeshes can only be
