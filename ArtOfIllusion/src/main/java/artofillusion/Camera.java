...
/**
 * The Camera class has two functions. First, it keeps track of the transformations between the
 * various coordinate systems used by the program. These include:
 * <p>
 * <ul>
 * <li>Object Coordinates are used for defining the shape of individual objects. Each object has its
 * own local coordinate system.</li>
 * <li>World Coordinates are used for defining the arrangement of objects to form a scene.</li>
 * <li>View Coordinates describe the positions of objects relative to the viewer. Specifically, the
 * viewer is considered to be at the origin, and to be looking along the positive z axis.</li>
 * <li>Screen coordinates correspond to pixels in the image being generated.</li>
 * </ul>
 * <p>
 * Second, the camera class is responsible for drawing various simple shapes. These include straight
 * lines, rectangular boxes, and Bezier curves.
 * </p>
 * <p>
 * The Camera class does *not* represent a "camera" which the user creates and positions within a
 * scene. That is represented by the SceneCamera class.
 * </p>
 */
public class Camera implements Cloneable {
     ...
     *
     * The distToScreen parameter controls the perpective strength. 'Screen' should be understood to
     * repserent the screen of a camera obscura, when the view is in perspective mode. A smaller
     * value means stronger perspective and vice versa. In parallel mode this parameter has no
     * meaning.
     *
    ...
    /**
     * The following routines return the various transformation matrices maintained by the Camera
     * object.
     */
    ...
    /**
     * Given a point in screen coordinates, find the corresponding point in world coordinates which
     * is at a specified depth from the viewpoint. If the grid is turned on, the oisuruib us
    ...
    /**
     * Given a point in screen coordinates, find the corresponding point in world coordinates which
     * is at a specified depth from the viewpoint. The optional parameter snapToGrid specifies
     * whether the position should be adjusted based on the current grid (if the grid is turned on).
     */
    ...
    /**
     * This is provided for backward compatibility. The version that takes doubles as arguments is
     * preferred.
     */
    ...
    /**
     * The following routine is used for dragging objects on the screen. Given a point p (specified
     * in world coordinates) which has been dragged by (dx, dy) pixels, find the displacement vector
    ...
    /**
     * Given a bounding box (specified in object coordinates), return a rectangle which describes
     * the object's position on the screen. If the object is not visible, return null.
     */
     ...
     * <ul>
     * <li>NOT_VISIBLE: The entire bounding box is offscreen. The object does not need to be
     * drawn.</li>
     * <li>NEEDS_CLIPPING: The object is partly visible, but at least one corner of the box lies in
     * front of the clipping plane. It should be drawn using the clipping drawing routines.</li>
     * <li>VISIBLE: The object is entirely in front of the viewer, and can be drawn with the faster
     * (non-clipping) drawing routines.</li>
     * </ul>
     */
    ...
    /**
     * Draw a cubic Bezier curve, using the four specified control vertices. We do this by forward
     * differencing. For speed, we always subdivide the curve into eight segments. This should be
     * enough for interactive purposes, and allows the forward differencing coefficients to be
     * precalculated.
     */
    ...
    /**
     * Draw a cubic Bezier curve, using the four specified control vertices. We do this by recursive
     * subdivision. We always subdivide three times, which should be enough for interactive
     * purposes. For speed, fixed point arithmetic is used throughout.
     */
