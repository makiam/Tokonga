...
public abstract class ObjectEditorWindow extends BFrame implements EditingWindow {
    ...
    protected final EditingWindow parentWindow;
    ...
    protected void recursivelyAddListeners(Widget w) {
        ...
        if (w instanceof WidgetContainer) {
            ((WidgetContainer) w).getChildren().forEach(widget -> recursivelyAddListeners(widget));
        }
    }
    ...
    public void setTemplateCommand() {
        ...
        try {
            ...
            MessageDialog.create().withOwner(this.getComponent()).error(Translate.text("errorLoadingImage", fc.getSelectedFile().getName()));
            return;
        }
