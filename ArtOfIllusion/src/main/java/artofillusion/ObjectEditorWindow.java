...
/**
 * The ObjectEditorWindow class represents a window for editing an object. This is an abstract
...
public abstract class ObjectEditorWindow extends BFrame implements EditingWindow {
    ...
    /**
     * Initialize various objects that will be used by the window. Subclasses typically invoke this
    ...
    /**
     * We need to be notified of key events which take place anywhere in the window. To make sure
     * this happens, we need to add the window as a KeyPressed listener to every component contained
     * in it.
     */
    protected void recursivelyAddListeners(Widget widget) {
        ...
        if (widget instanceof WidgetContainer) {
            ((WidgetContainer) widget).getChildren().forEach((Widget item) -> {
                recursivelyAddListeners(item);
            });
        }
    }
