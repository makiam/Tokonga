...
public class ColorDarkenModule extends ProceduralModule {
    ...
    public ColorDarkenModule(Point position) {
        super(Translate.text("menu.darkerModule"), new IOPort[]{new IOPort(IOPort.COLOR, IOPort.INPUT, IOPort.TOP, new String[]{"Color 1", '(' + Translate.text("white") + ')'}),
            new IOPort(IOPort.COLOR, IOPort.INPUT, IOPort.BOTTOM, new String[]{"Color 2", '(' + Translate.text("white") + ')'})},
                new IOPort[]{new IOPort(IOPort.COLOR, IOPort.OUTPUT, IOPort.RIGHT, new String[]{"Darker"})},
                position);
