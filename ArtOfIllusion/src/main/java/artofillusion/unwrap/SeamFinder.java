...
public class SeamFinder {
    ...
    private final ArrayList<Integer> seamEdges;
    ...
    private double computePatchDistortion(Vec3 vertexPos, Set<Integer> edges) {
        ...
        MeshVertex[] meshVertex = mesh.getVertices();
        TriangleMesh.Edge[] meshEdge = mesh.getEdges();
        Set<Integer> remainingEdges = new HashSet<>(edges);
        List<Set<Integer>> loopEdges = new ArrayList<>();
        List<Set<Integer>> loopVerts = new ArrayList<>();
        while (remainingEdges.size() > 0) {
            ...
            if (!foundEdge) {
                ...
                Set<Integer> newLoopEdges = new HashSet<>();
                Set<Integer> newLoopVerts = new HashSet<>();
    ...
    private void computeCostToVertex(final double[] costToVertex, int vertex, double[] edgeCost, int[][] vertEdges) {
        ...
        PriorityQueue<Integer> front = new PriorityQueue<>(10, new Comparator<>() {
            @Override
    ...
    private class Patch {
        ...
        final HashSet<Integer> candidates;
        final double[] costToVertex;
