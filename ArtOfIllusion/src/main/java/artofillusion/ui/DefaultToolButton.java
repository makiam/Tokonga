...
/**
 * This ToolButton is the classic button with one icon for standard representation and another one
 * for selected state.
 *
...
public class DefaultToolButton extends ToolButton {
     ...
     */
    /**
     * create a DefaultToolButton for the specified owner, and the icon specified by the
     * <i>image</i>, by applying the correct style to the image to generate theme-consistent
     * <i>normal</i> and <i>selected</i>
     * icons.
     *
     * This ctor is the preferred ctor, as it ensures the greatest consistency of all icons with the
     * theme.
     *
     * @param owner the owning object for this button
     *
     * @param image the image to use with the style to generate the normal and selected icons for
     * this button.
     */
     ...
     *
     * This is the least-preferred ctor, since it applies no consistency at all, and relies on the
     * icon creator to have made the icons consistent. This normally works well for icons that are
     * part of the theme, but usually works very poorly for icons associated with plugins.
     *
     ...
     *
     * If the style defined no <code>type.icon</code> attribute, then a definition is generated
     * using the <code>type.background</code> and <code>type.overlay</code> attributes. If no style
     * definition can be found or generated, the icon from <i>image</i> is returned.
     *
     * @param style the ButtonStyle to apply
     * @param type the type of icon to create. Current values are
     * <em>normal</em> and <em>selected</em>. In theory <em>highlighted</em> can also be specified,
     * but currently no code recognizes this type.
     *
     * @param owner the owner of this style.
     *
     * @param image the image to use with the style to generate the final icon.
     *
