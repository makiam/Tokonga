...
/**
 * This class provides utilities for localizing text so that it can be translated into different
 * languages. It does this by loading strings from a resource bundle, and using them to create
 * properly localized widgets.
 * <p>
 * The resource bundle is created from a
 * {@link artofillusion.PluginRegistry.PluginResource PluginResource} of type "TranslateBundle"
 * provided by the {@link artofillusion.PluginRegistry PluginRegistry}. By default it uses the
 * PluginResource with ID "artofillusion" which is built into the application, but you can specify a
 * different one by prefixing its ID to the property name passed to any method of this class. This
 * allows plugins to provide their own ResourceBundles for localizing their strings. To do this, the
 * plugin should include a set of properties files that define the localized versions of its
 * strings, such as:
 * <p>
...
public class Translate {
    ...
    /**
     * Get a BMenuItem whose text is given by the property "menu.(name)". If listener is not null,
     * the specified method of it will be added to the BMenuItem as an event link for CommandEvents,
     * and the menu item's action command will be set to (name). This also checks for a property
     * called "menu.shortcut.(name)", and if it is found, sets the menu shortcut accordingly.
     */
     ...
     * Get a BMenuItem whose text is given by the property "menu.(name)". If listener is not null,
     * the specified method of it will be added to the BMenuItem as an event link for CommandEvents,
     * and the menu item's action command will be set to (name). This form of the method allows you
     * to explicitly specify a menu shortcut, rather than using the one given in the properties
     * file.
     */
    ...
    /**
     * Get a BCheckBoxMenuItem whose text is given by the property "menu.(name)". If listener is not
     * null, the specified method of it will be added to the BCheckboxMenuItem as an event link for
     * CommandEvents. state specifies the initial state of the item.
     */
    ...
    /**
     * Get a BButton whose text is given by the property "button.(name)". If listener is not null,
     * the specified method of it will be added to the BButton as an event link for CommandEvents,
     * and the menu item's action command will be set to (name).
     */
    ...
    /**
     * Get a BButton whose text is given by the property "button.(name)", with a suffix appended to
     * it. If listener is not null, the specified method of it will be added to the BButton as an
     * event link for CommandEvents, and the button's action command will be set to (name).
     */
    ...
    /**
     * Get a BLabel whose text is given by the property "name". If the property is not found, this
     * simply uses name.
     */
    ...
    /**
     * Get a BLabel whose text is given by the property "name", with a suffix appended to it. If the
     ...
     * name. Any occurrance of the pattern "{0}" in the text string will be replaced with the string
     * representation of arg1.
     */
    ...
    /**
     * Get the text given by the property "name". If the property is not found, this simply uses
     * name. Any occurrances of the patterns "{0}" and "{1}" in the text string will be replaced
     * with the strings representations of arg1 and arg2, respectively.
     */
