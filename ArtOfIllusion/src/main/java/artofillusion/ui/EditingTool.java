...
/**
 * EditingTool is the superclass of tools for editing objects or scenes. An EditingTool has an image
 * which appears in a tool palette, allowing the tool to be selected. When selected, the editing
 * tool responds to events in the scene or object viewer.
 * <p>
 * <p>
 * Editing tool GUI properties can be set using Annotations from artofillusion.tool.annotations.*
 * package or corresponding methods can be overrided An EditingTool specifies what types of mouse
 * clicks it wants to receive by the value it returns from its whichClicks() method. This should be
 * a sum of the constants OBJECT_CLICKS (for mouse clicks on objects), HANDLE_CLICKS (for mouse
 * clicks on handles), and ALL_CLICKS (for all mouse clicks regardless of what they are on). The
 * exact definition of an "object" or "handle" is not specified. It is up to the ViewerCanvas
 * generating the events to decide what constitutes an object or handle.
 * <p>
 * An EditingTool may also specify whether the current selection may be changed while that tool is
 * active by the value it returns from its allowSelectionChanges() method. This method is always
 * called <i>after</i> mousePressed(), allowing the tool to first determine what was clicked on
 * before deciding whether to allow the selection to change in response to the click.
 * <p>
 * More precisely, here is the sequence of actions a ViewerCanvas performs when the mouse is
 * pressed:
 * <ol>
 * <li>If the active EditingTool has requested ALL_CLICKS, its mousePressed() method is
 * invoked.</li>
 ...
 * <li>If the click was on a handle and the active EditingTool has requested HANDLE_CLICKS, its
 * mousePressedOnHandle() method is invoked.</li>
 * <li>Otherwise, if the click was on an object and the active EditingTool has requested
 * OBJECT_CLICKS, its mousePressedOnObject() method is invoked.</li>
 * </ol>
 */
public abstract class EditingTool {
    ...
    /**
     * Get whether the selection may be changed while this tool is active. The default
     * implementation returns true if whichClicks() requests either OBJECT_CLICKS or HANDLE_CLICKS.
     */
