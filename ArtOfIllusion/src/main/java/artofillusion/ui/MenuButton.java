...
public class MenuButton extends JButton {
    ...
    public void updateUI() {
        super.updateUI();
        Optional.ofNullable(popupMenu).ifPresent(popupMenu -> SwingUtilities.updateComponentTreeUI(popupMenu));
    }
    ...
    private static class CompoundIcon implements Icon {
        ...
        private int paintArrowIcon(Component c, Graphics g, int x, int y) {
            var iconHeight = (c.getHeight() - arrowIcon.getIconHeight()) / 2;
            Graphics2D g2d = (Graphics2D) g;
            Object old = g2d.getRenderingHint(RenderingHints.KEY_ANTIALIASING);
            g2d.setRenderingHint(RenderingHints.KEY_ANTIALIASING, RenderingHints.VALUE_ANTIALIAS_ON);
            arrowIcon.paintIcon(c, g2d, x, iconHeight);
