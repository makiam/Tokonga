...
public class UIUtilities {
    ...
    public static void applyDefaultFont(Widget w) {
        ...
        if (w instanceof WidgetContainer && !(w instanceof BMenuBar)) {
            ((WidgetContainer) w).getChildren().forEach((Widget item) -> {
                applyDefaultFont(item);
            });
        }
    }

    /**
     * Set up a Widget and all of its children to have the default background and text colors for
     * the program.
     */
    ...
    /**
     * Given a Widget, find the window that contains it. If the Widget is not in a window, return
     * null.
     */
     ...
     * returned. If it is inside a BDialog, this returns the dialog's parent frame. Otherwise, this
     * returns null.
     */
    ...
    /**
     * Find every Widget which is contained within a specified one, either as a direct child or
    ...
    /**
     * Test event for specified mouse/modifier key combination. This regularizes 3-button and
     * 1-button + modifier support across multiple platforms.
     */
