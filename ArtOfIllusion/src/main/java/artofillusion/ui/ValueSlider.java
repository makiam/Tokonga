 ...
 * Editing either one causes the other to change automatically. If an illegal value is entered into
 * the BTextField, the text turns red to indicate this.
 */
public class ValueSlider extends WidgetContainer {
    ...
    /**
     * Layout the child Widgets. This may be invoked whenever something has changed (the size of
     * this WidgetContainer, the preferred size of one of its children, etc.) that causes the layout
     * to no longer be correct. If a child is itself a WidgetContainer, its layoutChildren() method
    ...
    /**
     * Get the smallest size at which this Widget can reasonably be drawn. When a WidgetContainer
     * lays out its contents, it will attempt never to make this Widget smaller than its minimum
     * size.
     */
    @Override
