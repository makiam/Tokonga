...
public class TriMeshEditorWindow extends MeshEditorWindow implements EditingWindow {
    ...
    private void createEditMenu() {
        ...
        editMenuItem[0] = Translate.menuItem("clear", event -> deleteCommand());
        if (topology) {
            editMenu.add(editMenuItem[0]);
            editMenu.addSeparator();
        }
        editMenu.add(editMenuItem[1] = Translate.menuItem("extendSelection", event -> extendSelectionCommand()));
        editMenu.add(editMenuItem[2] = Translate.menuItem("invertSelection", event -> invertSelectionCommand()));
        BMenu selectSpecialMenu = Translate.menu("selectSpecial");
        selectMenuItem = new BMenuItem[4];
        selectSpecialMenu.add(selectMenuItem[0] = Translate.menuItem("selectBoundary", event -> selectObjectBoundaryCommand()));
        selectSpecialMenu.add(selectMenuItem[1] = Translate.menuItem("selectSelectionBoundary", event -> selectSelectionBoundaryCommand()));
        selectSpecialMenu.add(selectMenuItem[2] = Translate.menuItem("selectEdgeLoop", event -> selectEdgeLoopCommand()));
        selectSpecialMenu.add(selectMenuItem[3] = Translate.menuItem("selectEdgeStrip", event -> selectEdgeStripCommand()));
        editMenu.add(Translate.menuItem("selectAll", event -> selectAllCommand()));
        editMenu.add(editMenuItem[3] = Translate.menuItem("deselectAll", event -> deselectAllCommand()));
        editMenu.addSeparator();
    ...
    void createMeshMenu(TriangleMesh obj) {
        ...
        meshMenuItem[0] = Translate.menuItem("subdivideEdges", event -> subdivideCommand());
        if (topology) {
            meshMenu.add(meshMenuItem[0]);
        }
        meshMenuItem[1] = Translate.menuItem("simplify", event -> simplifyCommand());
        if (topology) {
        ...
        meshMenuItem[5] = Translate.menuItem("bevel", event -> bevelCommand());
        if (topology) {
            meshMenu.add(meshMenuItem[5]);
        }
        meshMenu.add(meshMenuItem[6] = Translate.menuItem("parameters", event -> setParametersCommand()));
        if (topology) {
            meshMenu.add(Translate.menuItem("optimize", event -> optimizeCommand()));
        }
        meshMenu.add(Translate.menuItem("centerMesh", event -> centerCommand()));
        meshMenu.addSeparator();
        meshMenuItem[7] = Translate.menuItem("closeBoundary", event -> closeBoundaryCommand());
        if (topology) {
            meshMenu.add(meshMenuItem[7]);
        }
        meshMenuItem[8] = Translate.menuItem("joinBoundaries", event -> joinBoundariesCommand());
        if (topology) {
    ...
    public void updateMenus() {
        ...
        Joint selJoint = theMesh.getSkeleton().getJoint(view.getSelectedJoint());
    ...
    int[] findProjectedEdges() {
        ...
        if (!getProjectOntoSurface() || (mesh.getSmoothingMethod() != Mesh.APPROXIMATING && mesh.getSmoothingMethod() != Mesh.INTERPOLATING)) {
            lastPreview = null;
            return null;
        }
    ...
    private void findQuads() {
        ...
        // a rectangle.
        record EdgeScore(int edge, double score) implements Comparable<EdgeScore> {
    ...
    public void subdivideCommand() {
        ...
        if (selectMode == EDGE_MODE) {
            ...
            if (theMesh.getSmoothingMethod() == Mesh.APPROXIMATING) {
                newmesh = TriangleMesh.subdivideLoop(theMesh, selected, Double.MAX_VALUE);
            } else if (theMesh.getSmoothingMethod() == Mesh.INTERPOLATING) {
    ...
    private int[][] findSelectedBoundaries() {
        ...
        for (int[] ints: boundary) {
            // Add one "selected boundary" for every continuous run of selected edges.

            int start;
            for (start = ints.length - 1; start > 0 && selected[ints[start]]; start--);
            List<Integer> current = null;
            int j = start;
            do {
                boolean isSelected = selected[ints[j]];
                if (isSelected) {
                    if (current == null) {
                        current = new Vector<>();
                    }
    ...
    public void closeBoundaryCommand() {
        ...
        for (int[] ints : boundaryList) {
            ...
            Edge ed0 = ed[ints[0]];
            Edge ed1 = ed[ints[1]];
            boolean closed = isBoundaryClosed(ints);
            if (ints.length == 2 || (ints.length == 3 && closed)) {
                ...
                if (closed) {
                    center.scale(1.0 / ints.length);
                } else {
                    center.add(vt[j].r);
                    center.scale(1.0 / (ints.length + 1));
                }
        ...
        for (var edge : newed) {
            ...
            for (var value : ed) {
                if ((edge.v1 == value.v1 && edge.v2 == value.v2) || (edge.v1 == value.v2 && edge.v2 == value.v1)) {
                    edge.smoothness = value.smoothness;
                }
            }
        }
        setUndoRecord(new UndoRecord(this, false, UndoRecord.COPY_OBJECT, newmesh, theMesh));
    ...
    TriangleMesh doJoinBoundaries(int[][] boundary, int[][] boundaryVert, double offset, boolean reverse) {
        ...
        if (isBoundaryClosed(boundary[0])) {
            ...
            double p0 = 0.0;
            double p1 = (reverse ? boundary[1].length : 0.0);
            ...
            while (count < newface.length) {
                ...
                if (i0 != i0prev) {
                    ...
                    Face f = fc[e.f1];
                    int v1 = boundaryVert[0][i0prev];
                    int v2 = boundaryVert[0][i0];
                    if ((f.v1 == v1 && f.v2 == v2) || (f.v2 == v1 && f.v3 == v2) || (f.v3 == v1 && f.v1 == v2)) {
                        newface[count++] = new int[]{v2, v1, boundaryVert[1][i1prev]};
                    } else {
                        newface[count++] = new int[]{v1, v2, boundaryVert[1][i1prev]};
                ...
                if (i1 != i1prev) {
                    ...
                    Face f = fc[e.f1];
                    int v1 = boundaryVert[1][i1prev];
                    int v2 = boundaryVert[1][i1];
                    if ((f.v1 == v1 && f.v2 == v2) || (f.v2 == v1 && f.v3 == v2) || (f.v3 == v1 && f.v1 == v2)) {
                        newface[count++] = new int[]{v2, v1, boundaryVert[0][i0]};
                    } else {
                        newface[count++] = new int[]{v1, v2, boundaryVert[0][i0]};
    ...
    public void setSmoothnessCommand() {
        ...
        smoothness = new ValueSlider(0.0, 1.0, 100, value);
