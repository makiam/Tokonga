...
public class TexturedVertexShader implements VertexShader {
    ...
    /**
     * In some cases, a texture can be represented by a simpler shader. In addition, it is sometimes
     * possible to reuse shaders, thus avoiding having to repeat texture calculations. This method
     * returns a shader which produces identical results to this one, but may or may not actually be
     * the same object.
     */
    ...
    /**
     * Get whether this shader represents a uniform texture. If this returns true, all texture
     * properties are uniform over the entire surface (although different parts may still be colored
     * differently due to lighting).
     */
    @Override
