...
public class SoftwareCanvasDrawer implements CanvasDrawer {
     ...
     * Unlike the other methods of this class, this method may be called at arbitrary times (though
     * always from the event dispatch thread), not during the process of rendering the image.
     */
    @Override
     ...
     * calling super.updateImage() to make sure the Image exists and is the right size.
     */
     ...
     * rendering the image. So why does it work that way? First, because I was lazy and didn't want
     * to take the trouble to actually write a proper rasterizer. Second, because most people will
     * be using the OpenGL renderer instead. And third, because renderImage() is currently only used
     * for reference images, all of which means that the speed of this method won't be very
