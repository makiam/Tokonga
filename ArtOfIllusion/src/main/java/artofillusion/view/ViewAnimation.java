 ...
 *
 * ViewAnimation also takes care of sending a ViewChanged event and repainting the view after the
 * animation is done, so the calling method won't have to.
 */
public class ViewAnimation {
    ...
    /**
     * Start animation of perspective change. Camera position will not eventually chnage but the
     * camera will travel from/to infinity to produce the perspectve change effect.
     */
    ...
    public void start(CoordinateSystem endCoords, Vec3 endRotationCenter, double endScale, int endOrientation, int nextNavigation) {
        ...
        if (endAngles[0] - startAngles[0] < -180.0) {
            endAngles[0] += 360.0;
        }
        if (endAngles[1] - startAngles[1] < -180.0) {
            endAngles[1] += 360.0;
        }
        if (endAngles[2] - startAngles[2] < -180.0) {
            endAngles[2] += 360.0;
        }
        ...
        if (Math.abs(endAngles[1] - startAngles[1]) > angleMax) {
            angleMax = Math.abs(endAngles[1] - startAngles[1]);
        }
        if (Math.abs(endAngles[2] - startAngles[2]) > angleMax) {
    ...
    /**
     * Check if there is anything that should move. This is for the <b>non-perspectivechanging</b>
     * animations
     */
    private boolean noMove() {
        if (!rotStart.equals(endRotationCenter)) {
            return false;
        }
        if (!startCoords.getOrigin().equals(endCoords.getOrigin())) {
            return false;
        }
        if (startAngles[0] != endAngles[0]) {
            return false;
        }
        if (startAngles[1] != endAngles[1]) {
            return false;
        }
        if (startAngles[2] != endAngles[2]) {
            return false;
        }
        if (startScale != endScale) {
            return false;
