...
public class KeystrokeManager {
    ...
    /**
     * Read an XML file from an InputStream and add all the keystrokes it contains. For each one, it
     * checks whether there was already an existing keystroke with the same name. If so, the new
     * keystroke replaces the old one. If not, the new keystroke is simply added.
     */
    public static void addRecordsFromXML(InputStream in) throws Exception {
        ...
        for (KeystrokeRecord record : kr.items) {
            if (existing.containsKey(record.getName())) {
                keystrokes.items.remove(existing.get(record.getName()));
            }
            addRecord(record);
        }
    }
