...
public class UVMappingManipulator {
    ...
    public void mousePressed(WidgetMouseEvent ev) {
        ...
        if (ev.isControlDown()
                ...
                && boxes[CENTER_HANDLE].width + 6 >= click.x - boxes[CENTER_HANDLE].x && boxes[CENTER_HANDLE].y <= click.y + 3
        ...
        for (int i = 0; i < v.length; i++) {
            if (click.x > v[i].x - 3 && click.x < v[i].x + 3 && click.y > v[i].y - 3 && click.y < v[i].y + 3) {
                hit = true;
                if (selected[i]) {
        ...
        if (hit && !dragging) {
            ...
            for (int i = 0; i < boxes.length; i++) {
                if (boxes[i].x <= click.x + 3
                    ...
                    handle = i;
                    if (handle == US_HANDLE || handle == VS_HANDLE || handle == UVS_HANDLE) {
        ...
        if (dragging) {
            switch (handle) {
                ...
                case UVS_HANDLE:
                    ...
                    Vec2 current = new Vec2(currentPt.x - center.x, currentPt.y - center.y);
                ...
                case ROTATE_HANDLE:
                    ...
                    if (window.tensionOn()) {
                        for (int i = 0; i < v.length; i++) {
                            if (selected[i] && !canvas.isPinned(i)) {
                                x = cs * (originalPositions[i].x - center.x) + sn * (originalPositions[i].y - center.y);
                                y = -sn * (originalPositions[i].x - center.x) + cs * (originalPositions[i].y - center.y);
                                deltas[i].x = x + center.x - originalPositions[i].x;
                                deltas[i].y = y + center.y - originalPositions[i].y;
                            } else {
                                deltas[i].x = 0;
            ...
            if (!globalMoving) {
                canvas.setOrigin(originalOrigin.x - (currentPt.x - click.x) / originalScale,
                        originalOrigin.y + (currentPt.y - click.y) / originalScale);
                canvas.repaint();
            } else {
    ...
    public void mouseReleased(WidgetMouseEvent ev) {
        ...
        if (scaling || moving) {
            if (globalMoving || globalScaling) {
                UVMappingCanvas.MappingPositionsCommand cmd = canvas.new MappingPositionsCommand(originalVertices, null);
        ...
        if (selChange != null) {
            canvas.getParentDialog().addUndoCommand(canvas.new SelectionCommand(selChange));
        }
        if (dragBox || dragging) {
            if (dragging) {
                int count = 0;
                ...
                window.addUndoCommand(canvas.new DragMappingVerticesCommand(vertIndices,
                        undoPositions,
                        redoPositions,
                        canvas.getMapping(),
                        canvas.getSelectedPiece()));
            }
    ...
    public void paint(Graphics2D g) {
        ...
        g.drawLine(center.x + dx, center.y + dy, center.x + dx, center.y - axisLength + dy);
        ...
        if (dragging) {
            ...
            switch (handle) {
                ...
                case US_HANDLE:
                    ...
                    g.drawImage(ghostscale,
                            center.x + sdx + 2 * IMAGE_MARGIN + HANDLE_SIZE,
                ...
                case VS_HANDLE:
                    ...
                    g.drawImage(ghostscale,
                            ...
                            center.y - sdy - 2 * IMAGE_MARGIN - 2 * HANDLE_SIZE,
                            canvas.getComponent());
                    break;
                ...
                case UVS_HANDLE:
                    ...
                    deltax = (int) Math.round((2 * IMAGE_MARGIN + HANDLE_SIZE) * sx - (1 - Math.abs(sy)) * HANDLE_SIZE / 2);
                    deltay = (int) Math.round((2 * IMAGE_MARGIN + 2 * HANDLE_SIZE) * sy + (1 - Math.abs(sx)) * HANDLE_SIZE / 2);
                    g.drawLine(center.x, center.y, center.x + sdx, center.y - sdy);
                    g.drawImage(ghostscale, center.x + sdx + deltax, center.y - sdy - deltay, canvas.getComponent());
                    break;
            }
        }
    }
