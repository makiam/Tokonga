...
public class MeshUnfolder {
     ...
     *
     * @param mesh The mesh to unfold (any kind of faceted mesh)
     * @param trimesh The triangle mesh version of original mesh (maybe equal to mesh if mesh is a
     * triangle mesh).
     * @param vertexTable Table to vertex indices prior to opening seams. Correct vertex index will
     * be placed in UnfoldedVertex data. If a the vertexTable length is smaller than the number of
     * vertices, then these vertices are supposed to be created for triangulation purposes and will
     * be assigned a vertex index of -1. They won't be displayed by the UVMappingCanvas widget. This
     * array may be null if the mesh is a triangle mesh has no seams.
     * @param faceTable Table to face indices. If this array is not null, then unfoldedFaces will be
     * given an id corresponding to this array. This allows to identify which faceted face yielded a
     * given trimesh face. This array may be null if the mesh to unfold is already a triangle mesh.
     * Correct vertex index will be placed in UnfoldedVertex data. If a the vertexTable length is
     * smaller than the number of vertices, then these vertices are supposed to be created for
     * triangulation purposes and will be assigned a vertex index of -1. They won't be displayed by
     * the UVMappingCanvas widget.
     */
    ...
    /**
     * Given a piece of the unfolded mesh, this method computes a standalone unfolded mesh. Ids are
     * kept, they will make it possible to find back which face/vertex is concerned during UVMapping
     * edition.
     */
...
//			v3 = faces[i].v3;
    ...
    /**
     * Given an edge and a face, this method checks the 3rd vertex as being unfolded if's not
     * already checked
     */
