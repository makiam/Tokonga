...
public final class PolyMesh extends Object3D implements FacetedMesh {
    ...
    void setVertices(Wvertex[] vertices) {
    ...
    //smoothness levels applied before display (interactive) or triangular smoothing (rendering)
    ...
    public PolyMesh(int type, int u, int v, double sx, double sy, double sz) {
        ...
        switch (type) {
            case 0:
                // Cube
                PolyMeshBuilder.buildCubeMesh(this);
                break;
            case 2:
                // octahedron
                PolyMeshBuilder.buildOctahedronMesh(this);
                break;
            case 3:
                // cylinder
                if (u < 3) {
    ...
    public PolyMesh(SplineMesh smesh) {
        ...
        if (smesh.isUClosed()) {
            ...
            for (int i = 0; i < vSize - 1; i++) {
                ...
                int ele = -1;
                for (int k : el) {
                    if (edges[k].vertex == vertTable[0][i]) {
                        ele = k;
                    }
                }
                int ere = -1;
                for (int k : er) {
                    if (edges[k].vertex == vertTable[uSize - 1][i + 1]) {
                        ere = k;
                    }
                }
            ...
            int elefte = -1;
            for (int k : eleft) {
                if (edges[k].vertex == vertTable[1][0]) {
                    elefte = k;
                }
            }
            int erighte = -1;
            for (int k : eright) {
                if (edges[k].vertex == vertTable[uSize - 2][0]) {
                    erighte = k;
                }
            }
            ...
            elefte = -1;
            for (int k : eleft) {
                if (edges[k].vertex == vertTable[1][vSize - 1]) {
                    elefte = k;
                }
            }
            erighte = -1;
            for (int k : eright) {
                if (edges[k].vertex == vertTable[uSize - 2][vSize - 1]) {
                    erighte = k;
                }
            }
        ...
        if (smesh.isVClosed()) {
            ...
            for (int i = 0; i < us; i++) {
                ...
                int ede = -1;
                for (int value : ed) {
                    if (edges[value].vertex == vertTable[k][0]) {
                        ede = value;
                    }
                }
                int eue = -1;
                for (int value : eu) {
                    if (edges[value].vertex == vertTable[i][vSize - 1]) {
                        eue = value;
                    }
                }
            ...
            if (!smesh.isUClosed()) {
                ...
                int edowne = -1;
                for (int i : edown) {
                    if (edges[i].vertex == vertTable[0][1]) {
                        edowne = i;
                    }
                }
                int eupe = -1;
                for (int i : eup) {
                    if (edges[i].vertex == vertTable[0][vSize - 2]) {
                        eupe = i;
                    }
                }
                ...
                edowne = -1;
                for (int i : edown) {
                    if (edges[i].vertex == vertTable[uSize - 1][1]) {
                        edowne = i;
                    }
                }
                eupe = -1;
                for (int i : eup) {
                    if (edges[i].vertex == vertTable[uSize - 1][vSize - 2]) {
                        eupe = i;
                    }
                }
    ...
    public PolyMesh(TriangleMesh trimesh, boolean findQuads,  boolean angularSearch) {
        ...
        while (doQuads) {
            ...
            for (int i = 0; i < faces.length; ++i) {
                ...
                int face = -1;
                for (int k : fe) {
                    length = vertices[edges[k].vertex].r.distance(vertices[edges[edges[k].hedge].vertex].r);
                    f = edges[edges[k].hedge].face;
                    if (f == -1) {
                        continue;
                    }
     ...
     * @param vertex the index of the vertex within the face (between 0 and
    ...
    private void updateResizedMesh() {
        ...
        for (var newVertex : newVertices) {
            newVertex.edge = edgeTable[newVertex.edge];
        }
        for (var face : faces) {
            face.edge = edgeTable[face.edge];
        }
        vertices = newVertices;
        edges = newEdges;
    }
    ...
    private void planarMesh(int uSize, int vSize, double uMax, double vMax) {
        ...
        vertices[uSize * (vSize - 1)].edge = (euSize * evSize) * 2 + euSize + evSize - 1;
        for (int i = 0; i < euSize; i++) {
            k = i + uSize * (vSize - 1);
            ec = euSize * evSize * 2 + i;
            fd = i + euSize * (evSize - 1);
            edges[ec] = new Wedge(k, ec + el, fd, (i + euSize * (evSize - 1)) * 2 - 1 + el);
            if (i == 0) {
    ...
    public PolyMesh(Vec3[] v, int[][] faceIndices) {
        ...
        for (var vertex : vertices) {
            vertex.edge = edgeTable[vertex.edge];
        }
    ...
    public RenderingMesh getRenderingMesh(double tol, boolean interactive, ObjectInfo info) {
        ...
        Arrays.setAll(vertArray, i -> vert.get(i));
    ...
    public boolean[] triangulateFaces(boolean[] selected) {
        ...
        if (oldParamVal != null) {
            ...
            for (int k = 0; k < oldParamVal.length; k++) {
                ...
                } else if (oldParamVal[k] instanceof FaceVertexParameterValue fvpv) {
            ...
            if (ve.length > 1) {
                ...
                if (!added) {
                    for (int k : ve) {
                        ...
                        norm[i].add(faceNormals[edges[k].face]);
                    }
                }
                norm[i].normalize();
            } else {
    ...
    /**
     * Get an array of normal vectors, one for each edge.
     *
     * @return The normals value
     */
    public Vec3[] getEdgeNormals() {
        ...
        for (int i = 0; i < edges.length / 2; i++) {
            ...
            if (edges[i].face != -1) {
                vn = vertices[vi].r.minus(vertices[hvi].r).cross(vertices[nvi].r.minus(vertices[hvi].r));
                if (vn.length() > 1e-6) {
                    vn.normalize();
                } else {
            ...
            if (edges[edges[i].hedge].face != -1) {
                hvn = vertices[phvi].r.minus(vertices[hvi].r).cross(vertices[vi].r.minus(vertices[hvi].r));
                if (hvn.length() > 1e-6) {
                    hvn.normalize();
                } else {
            ...
            if (edges[i].face != -1) {
                vn = vertices[pvi].r.minus(vertices[vi].r).cross(vertices[hvi].r.minus(vertices[vi].r));
                if (vn.length() > 1e-6) {
                    vn.normalize();
                } else {
                    vn = faceNormals[edges[i].face];
            ...
            if (edges[edges[i].hedge].face != -1) {
                hvn = vertices[hvi].r.minus(vertices[vi].r).cross(vertices[nhvi].r.minus(vertices[vi].r));
                if (hvn.length() > 1e-6) {
                    hvn.normalize();
                } else {
                    hvn = faceNormals[edges[edges[i].hedge].face];
    ...
    public Vec3 getEdgePosition(int edge) {
        Vec3 middle = vertices[edges[edge].vertex].r.plus(vertices[edges[edges[edge].hedge].vertex].r);
        middle.scale(0.5);
        return middle;
    }
     ...
     * @param sSize The new size value
     */
    @Override
    public void setSize(double xSize, double ySize, double sSize) {
        ...
        if (smoothingMethod != Mesh.NO_SMOOTHING) {
            ...
            if (controlledSmoothing) {
                ...
                for (int i = 0; i < edge.length; i++) {
                    ...
                    f1 = triface[ed.f1];
                    f2 = triface[ed.f2];
                    norm1 = trivert[f1.v1].r.minus(trivert[f1.v2].r).cross(trivert[f1.v1].r.minus(trivert[f1.v3].r));
                    norm2 = trivert[f2.v1].r.minus(trivert[f2.v2].r).cross(trivert[f2.v1].r.minus(trivert[f2.v3].r));
                    norm1.normalize();
                    norm2.normalize();
                    dot = norm1.dot(norm2);
                    if (dot <= minA) {
                        ...
                        ed.smoothness = (float) ((dot - minA) / (maxA - minA)) * (minSmoothness - maxSmoothness) + maxSmoothness;
                    }
                }
            }
        }
     ...
     */
    public void deleteVertices(int[] selectedIndices) {
        ...
        for (int k : selectedIndices) {
            deletedVertices[k] = true;

            for (int i : getVertexEdges(vertices[k])) {
                deletedEdges[edges[i].hedge] = true;
                deletedEdges[i] = true;
                if (edges[edges[i].hedge].face != -1) {
                    deletedFaces[edges[edges[i].hedge].face] = true;
                }
                if (edges[i].face != -1) {
                    deletedFaces[edges[i].face] = true;
                }
            }
        }
    ...
    private int[] deletion(boolean[] deletedVertices, boolean[] deletedEdges, boolean[] deletedFaces, boolean mirrorOp) {
        ...
        while (newDelete) {
            ...
            for (int i = 0; i < edges.length; ++i) {
                if ((!deletedEdges[i]) && deletedEdges[edges[i].next]) {
                    ...
                    count = 0;
                    for (int k : getVertexEdges(vertices[edges[i].vertex])) {
                        if (!deletedEdges[k]) {
                            ++count;
                        }
                    }
        ...
        for (int i = 0; i < vertices.length; ++i) {
            ...
            count = 0;
            for (int k : getVertexEdges(vertices[i])) {
                if (!deletedEdges[k]) {
                    ++count;
        ...
        count = 0;
        for (boolean deletedVertex : deletedVertices) {
            if (deletedVertex) {
                count++;
            }
        }
        ...
        count = 0;
        for (boolean deletedFace : deletedFaces) {
            if (deletedFace) {
                count++;
            }
        }
        ...
        if (mirrorOp) {
            if (mirroredVertices == null) {
                ...
                int[] newInvMirroredVertices = new int[count];

                for (int i = 0; i < invMirroredVertices.length; ++i) {
                    if (vertexTable[i] != -1) {
                        newInvMirroredVertices[vertexTable[i]] = invMirroredVertices[i];
                    }
                }
                count = 0;
                ...
                invMirroredVertices = newInvMirroredVertices;
     ...
     */
    public void deleteEdges(int[] edgeIndices) {
        boolean[] deletedVertices = new boolean[vertices.length];
        boolean[] deletedEdges = new boolean[edges.length];
        boolean[] deletedFaces = new boolean[faces.length];

        for (int edgeIndex : edgeIndices) {
            deletedEdges[edges[edgeIndex].hedge] = true;
            deletedEdges[edgeIndex] = true;
            if (edges[edges[edgeIndex].hedge].face != -1) {
                deletedFaces[edges[edges[edgeIndex].hedge].face] = true;
            }
            if (edges[edgeIndex].face != -1) {
                deletedFaces[edges[edgeIndex].face] = true;
            }
        }
        boolean del1;
        boolean del2;
    ...
    public boolean[] divideEdges(boolean[] sel, int nseg) {
        ...
        for (int i = 0; i < sel.length; ++i) {
            if (sel[i]) {
                ...
                Vec3 delta = newVertices[to].r.minus(newVertices[from].r);
                ...
                newVertices[vl + index] = new Wvertex(vertices[from].r.plus(delta), el + index);
                newEdges[i].vertex = vl + index;
                newEdges[i].next = el + index;
                newEdges[el + index] = new Wedge(to, el + index + newEdges.length / 2, faceTo, edgeTo);
                newEdges[el + index + newEdges.length / 2] = new Wedge(vl + index, el + index, faceFrom, newEdges[i].hedge);
                ...
                for (int j = 1; j < add; ++j) {
                    ++index;
                    newVertices[vl + index] = new Wvertex(vertices[from].r.plus(delta.times(j + 1)), el + index);
                    newEdges[el + index - 1].vertex = vl + index;
                    newEdges[el + index - 1].next = el + index;
                    newEdges[el + index] = new Wedge(to, el + index + newEdges.length / 2, faceTo, edgeTo);
                    newEdges[el + index + newEdges.length / 2] = new Wedge(vl + index, el + index, faceFrom, newEdges[el + index - 1].hedge);
                    newEdges[el + index].smoothness = edges[i].smoothness;
                    newEdges[el + index + newEdges.length / 2].smoothness = edges[i].smoothness;
                    newVertices[to].edge = el + index + newEdges.length / 2;
    ...
    public boolean[] mergeEdges(int e1, int e2, boolean center) {
        ...
        if (ne2 == e1) {
            ...
            for (int j : ve) {
                edges[edges[j].hedge].vertex = edges[he1].vertex;
            }
        }
        if (re1 == -1) {
            edges[pe2].next = ne1;
     ...
     *
     * @param e1
     * First edge to merge within selection
     * @param e2
     * Second edge to merge within selection
     * @param sel
     * Boundary edges selection (may be null)
    ...
    public boolean[] mergeEdges(int e1, int e2, boolean[] sel, boolean center) {
        ...
        if (closed1 && closed2) {
            ...
            for (int i = 0; i < l; i++) {
                ...
                if (center) {
                    vertices[edges[edges[e1].hedge].vertex].r.add(vertices[verts[i]].r);
                    vertices[edges[edges[e1].hedge].vertex].r.scale(0.5);
                }

                for (int k : getVertexEdges(vertices[verts[i]])) {
                    edges[edges[k].hedge].vertex = edges[edges[e1].hedge].vertex;
                }
                edges[phe2].next = e1;
                edges[e1].face = edges[he2].face;
                edges[e1].next = edges[he2].next;
            ...
            for (int i = 0; i < edges2.length; ++i) {
                ...
                for (int k : getVertexEdges(vertices[v])) {
                    edges[edges[k].hedge].vertex = edges[edges[edges1[i]].hedge].vertex;
                }
            }
            ...
            for (int k :  getVertexEdges(vertices[v])) {
                edges[edges[k].hedge].vertex = edges[edges1[edges1.length - 1]].vertex;
            }
            if (re1 == -1) {
                edges[pe2].next = ne1;
            } else {
            ...
            for (int i = 0; i < edges.length; i++) {
                deleted = false;
                for (int k : edges2) {
                    if (i == k || i == edges[k].hedge || i == re1 || i == re2 || i == hre1 || i == hre2) {
                        deleted = true;
                    }
                }
                if (deleted) {
    ...
    public void removeTwoEdgeBoundaries() {
        ...
        Wedge[] newEdges = new Wedge[count];
        count = 0;
        for (int i = 0; i < edges.length; ++i) {
            if (!deleted[i]) {
                newEdges[count++] = new Wedge(edges[i]);
    ...
    public ArrayList extractCurveFromSelection(boolean[] sel) {
        ...
        for (int i = 0; i < edges.length / 2; i++) {
            if (sel[i]) {
                ...
                for (int value : getVertexEdges(vertices[edges[i].vertex])) {
                    if (value != i && value != edges[i].hedge && isEdgeSelected(value, sel)) {
                        selected++;
                        regular = true;
                    }
                }

                for (int k : getVertexEdges(vertices[edges[edges[i].hedge].vertex])) {
                    if (k != i && k != edges[i].hedge && isEdgeSelected(k, sel)) {
                        selected++;
                    }
                }
                if (selected >= 2) {
                ...
                while (isEdgeSelected(p, sel)) {
                    ...
                    for (int k : getVertexEdges(vertices[edges[p].vertex])) {
                        if (k != p && k != edges[p].hedge && isEdgeSelected(k, sel)) {
                            p = k;
                        }
                    }
    ...
    public boolean[] divideEdges(boolean[] sel, double fraction) {
        ...
        for (int i = 0; i < sel.length; ++i) {
            if (sel[i]) {
                count++;
                v = vertices[edges[i].vertex].r.minus(vertices[edges[edges[i].hedge].vertex].r);
    ...
    public void connectVertices(int[] connect) {
        ...
        for (int i = 1; i < connect.length; ++i) {
            for (int j = 0; j < i; ++j) {
                ...
                for (int item : e1) {
                    for (int value : e2) {
                        if (edges[item].face == edges[value].face) {
                            if (edges[item].face != -1) {
                                face = edges[item].face;
                                add = true;
                            }
                        }
                    }
        ...
        int e2next = 0;
        int e2prev = 0;
        for (int value : getVertexEdges(vertices[i1min])) {
            if (edges[value].face == faceMin) {
                e1next = value;
            }
            if (edges[edges[value].hedge].face == faceMin) {
                e1prev = edges[value].hedge;
            }
        }
        for (int k : getVertexEdges(vertices[i2min])) {
        ...
        newEdges[newEdge] = new Wedge(i1min, newEdges.length - 1, faceMin, e1next);
        newEdges[newEdges.length - 1] = new Wedge(i2min, newEdge, faceMin, e2next);
            ...
            if (vertEdges.length == 4) {
                ...
                if (v.size() < 2) {
                    ...
                    for (int vertEdge : vertEdges) {
                        if (vertEdge == edges[currentEdge].hedge) {
                            continue;
                        }
                        Vec3 dir2 = vertices[edges[vertEdge].vertex].r.minus(vertices[edges[edges[vertEdge].hedge].vertex].r);
                        dir2.normalize();
                        double dot = dir1.dot(dir2);
                        if (edges[currentEdge].vertex == edges[vertEdge].vertex || edges[currentEdge].vertex == edges[vertEdge].vertex) {
                            dot = -dot;
                        }
                        if (dot > maxDot) {
                            maxDot = dot;
     ...
     */
    private boolean[] getSelectionFromMirror(PolyMesh mirroredMesh, boolean[] mirrorSel) {
        ...
        for (int i = 0; i < edges.length / 2; ++i) {
            Vec3 v1 = vertices[edges[i].vertex].r;
            Vec3 v2 = vertices[edges[edges[i].hedge].vertex].r;
            if (v1.distance(mirrorVerts[mirrorEdges[i].vertex].r) < 1.0e-6 && v2.distance(mirrorVerts[mirrorEdges[mirrorEdges[i].hedge].vertex].r) < 1.0e-6) {
                sel[i] = mirrorSel[i];
            } else {
                for (int j = 0; j < mirrorEdges.length / 2; ++j) {
                    if (v1.distance(mirrorVerts[mirrorEdges[j].vertex].r) < 1.0e-6 && v2.distance(mirrorVerts[mirrorEdges[mirrorEdges[j].hedge].vertex].r) < 1.0e-6) {
                        sel[i] = mirrorSel[j];
                        break;
                    }
                }
     ...
     */
    private boolean[] getMirroredSelection(PolyMesh mirroredMesh, boolean[] selection) {
        MeshVertex[] mirrorVerts = mirroredMesh.getVertices();
        Wedge[] mirrorEdges = mirroredMesh.getEdges();
        ...
        for (int i = 0; i < edges.length / 2; ++i) {
            ...
            if (v1.distance(vertices[edges[i].vertex].r) < 1.0e-6 && v2.distance(vertices[edges[edges[i].hedge].vertex].r) < 1.0e-6) {
                mirrorSel[i] = selection[i];
            } else {
                for (int j = 0; j < edges.length / 2; ++j) {
                    if (v1.distance(vertices[edges[j].vertex].r) < 1.0e-6 && v2.distance(vertices[edges[edges[j].hedge].vertex].r) < 1.0e-6) {
        ...
        if (mirrorState != NO_MIRROR) {
            ...
            for (int i = 0; i < edges.length / 2; i++) {
                ...
                if ((mirrorState & MIRROR_ON_XY) != 0) {
                    if (Math.abs(vertices[edges[i].vertex].r.z) < 1e-6 && Math.abs(vertices[edges[edges[i].hedge].vertex].r.z) < 1e-6) {
                        edgeSmoothness[i] = 0.0f;
                        edgeSmoothness[edges[i].hedge] = 0.0f;
                    }
                }
                if ((mirrorState & MIRROR_ON_YZ) != 0) {
                    if (Math.abs(vertices[edges[i].vertex].r.x) < 1e-6 && Math.abs(vertices[edges[edges[i].hedge].vertex].r.x) < 1e-6) {
                        edgeSmoothness[i] = 0.0f;
                        edgeSmoothness[edges[i].hedge] = 0.0f;
                    }
                }
                if ((mirrorState & MIRROR_ON_XZ) != 0) {
                    if (Math.abs(vertices[edges[i].vertex].r.y) < 1e-6 && Math.abs(vertices[edges[edges[i].hedge].vertex].r.y) < 1e-6) {
                        edgeSmoothness[i] = 0.0f;
                        edgeSmoothness[edges[i].hedge] = 0.0f;
                    }
                }
                    ...
                    for (int j = 0; j < newFaces.length; ++j) {
                        ...
                        for (int k = 0; k < newval[j].length; k++) {
                            if (newFaceVertFaceRef[j][k] != -1) {
                                // old vertex
                                newval[j][k] = fvpv.getValue(newFaceVertFaceRef[j][k], newFaceVertIndexRef[j][k]);
                            } else {
                                // new vertex
                                orFace = newToOldFaceIndex[j];
                                val = 0;
    ...
    public void smooth(boolean[] selected, boolean subdivideOnly) {
        ...
        for (int i = 0; i < faces.length; ++i) {
            if (selectType[i] > 0) {
                ++faceNum;
                for (int k : getFaceEdges(faces[i])) {
                    index = k;
        ...
        for (int i = 0; i < faces.length; ++i) {
            if (selectType[i] == 0) {
                ...
                if (newFaces[count].edge >= edges.length / 2) {
                    newFaces[count].edge += newEdges.length / 2 - edges.length / 2;
                }
        ...
        if (!subdivideOnly) {
            for (int i = 0; i < originalVert; ++i) {
                ...
                for (int k : ve) {
                    face1 = edges[k].face;
                    face2 = edges[edges[k].hedge].face;
                    if (face1 != -1) {
                ...
                pt = new Vec3();
                for (int k : ve) {
                    pt.add(vertices[edges[k].vertex].r);
                }
    ...
    public void moveEdges(boolean[] selected, double amount, short direction) {
        ...
        switch (direction) {
            case NORMAL:
                ...
                Vec3[] v = getVertexPositionsForFacesNormalDisplacement(facesSelected, amount);
                for (int i = 0; i < edges.length / 2; ++i) {
                    if (selected[i]) {
    ...
    public void moveFaces(boolean[] selected, double amount, short direction) {
        ...
        switch (direction) {
            case NORMAL:
                Vec3[] v = getVertexPositionsForFacesNormalDisplacement(selected, amount);
                for (int i = 0; i < vertices.length; ++i) {
                    vertices[i].r = v[i];
                }
                break;
     ...
     */
    public void extrudeFaces(boolean[] selected, double value, Vec3 direction, double scale, Vec3 camZ, boolean useNormals, boolean constrainAxis) {
        ...
        for (int i = 0; i < faces.length; ++i) {
            if (selected[i]) {
                ...
                for (int j = 0; j < fv.length; ++j) {
                    ...
                    newVertices[newVertCount + j] = new Wvertex(vertices[fv[j]].r, newEdgeCount + 2 * j + 1);
                    ...
                    newEdges[newEdgeCount + 2 * j + 1 + newEdges.length / 2] = new Wedge(newVertCount + j, newEdgeCount + 2 * j + 1, i, newEdgeCount + 2 * next + 1 + newEdges.length / 2);
        ...
        for (int i = 0; i < fl; ++i) {
            if (selected[i]) {
                ...
                int[] fv = getFaceVertices(faces[i]);
                for (int k : fv) {
                    diff.add(vertices[k].r);
                }
                diff.scale(1.0 / fv.length);
                for (int k : fv) {
                    vertices[k].r = diff.plus(vertices[k].r.minus(diff).times(scale));
                }
            }
        }
     ...
     * is not selected or is embedded in the selection (i.e. not bordered by an
    ...
    public void extrudeRegion(boolean[] selected, double value, Vec3 direction, double scale, Vec3 camZ, boolean useNormals, boolean constrainAxis) {
        ...
        for (int i = 0; i < faces.length; ++i) {
            if (selected[i]) {
                ...
                for (int item: getFaceEdges(faces[i])) {
                    int k = edges[edges[item].hedge].face;
                    if (k == -1) {
                        continue;
                    }
                    if (selected[k]) {
        ...
        for (int i = 0; i < vl; ++i) {
            ...
            if (done[i]) {
                int start = -1;
                for (int item : getVertexEdges(vertices[i])) {
                    if (isFaceSelected(selected, edges[item].face) && !isFaceSelected(selected, edges[edges[item].hedge].face)) {
                        start = item;
                    }
                    if (isFaceSelected(selected, edges[item].face) && (mean == null)) {
                        List<Integer> vv = sclFace[edges[item].face];
                        mean = new Vec3();
                        for (Integer index : vv) {
                            mean.add(sclFaceVert[index]);
                        }
                ...
                for (int j = 0; j < fc.length; ++j) {
                    ...
                    newEdges[newEdgeCount + 2 * j] = new Wedge(newVertCount + j, newEdgeCount + 2 * j + newEdges.length / 2, newFaceCount + j, newEdgeCount + 2 * j + 1);
        ...
        for (int i = 0; i < edges.length / 2; i++) {
            ...
            if (direction == null) {
                if (isEdgeSelected(edges[current].next, selected)) {
                    ...
                    newVertices[vindex].r.add(normals[edges[current].next].times(value * 0.5));
                } else {
                    newVertices[vindex].r.add(normals[current].times(value));
                }
                if (isEdgeSelected(previous, selected)) {
                    newVertices[vindex + 1].r.add(normals[current].times(value * 0.5));
                    newVertices[vindex + 1].r.add(normals[previous].times(value * 0.5));
                } else {
                    newVertices[vindex + 1].r.add(normals[current].times(value));
                }
            } else {
                newVertices[vindex].r.add(disp);
                newVertices[vindex + 1].r.add(disp);
            ...
            while (isEdgeSelected(edges[e].next, tmpSel)) {
                ...
                if (!closed || e != laste) {
                    ...
                    // special case, we have to close the extrusion
    ...
    public void thickenMesh(double value, boolean faceDisplacement) {
        ...
        for (int e : boundaries) {
            ...
            newEdges[index] = new Wedge(vve, index + newEdges.length / 2, -2, ee);
     ...
     */
    public Vec3[] getVertexPositionsForFacesNormalDisplacement(boolean[] selected, double amount) {
        Vec3[] v = new Vec3[vertices.length];
        Vec3[] faceNormals = getFaceNormals();
        ...
        for (int i = 0; i < faces.length; ++i) {
            ...
            int[] vf = getFaceVertices(faces[i]);
            for (int k : vf) {
                faceCenters[i].add(vertices[k].r);
            }
            ...
            if (selected[i]) {
                ...
                for (Wedge edge : edges) {
                    ...
                    while (checkNext) {
                        ...
                        for (int j : ve) {
                            if (edge.next == edges[j].hedge) {
                                edge.next = edges[edge.next].next;
                                checkNext = true;
                            } else if (edge.next == j) {
                                edge.next = edges[j].next;
                                checkNext = true;
                            }
                        }
                    }
        ...
        for (int i = 0; i < selected.length; ++i) {
            if (selected[i]) {
                ...
                for (int j = 0; j < faces.length; ++j) {
                    newFaces[j] = new Wface(faces[j]);
                    for (int value : ve) {
                        if (newFaces[j].edge == value) {
                            newFaces[j].edge = edges[newFaces[j].edge].next;
                        } else if (newFaces[j].edge == edges[value].hedge) {
                            newFaces[j].edge = edges[edges[edges[value].hedge].next].next;
                        }
                    }
                }
                ...
                if (oldParamVal != null) {
                    ...
                    for (int k = 0; k < oldParamVal.length; k++) {
                        if (oldParamVal[k] instanceof FaceParameterValue) {
                            ...
                            for (int value : ve) {
                                newval[newval.length - 1] += oldval[edges[value].face];
                            }
    ...
    private boolean[] deleteDanglingVertex(int vert, boolean[] selected) {
        ...
        int count = 0;
        for (int i = 0; i < edges.length; ++i) {
            if (i != ve[0] && i != edges[ve[0]].hedge && i != ve[1] && i != edges[ve[1]].hedge) {
                newEdges[count] = new Wedge(edges[i]);
        ...
        for (Wedge newEdge : newEdges) {
            newEdge.next = edgeTable[newEdge.next];
            if (newEdge.face > removedFace) {
                --newEdge.face;
            }
            newEdge.hedge = edgeTable[newEdge.hedge];
            if (newEdge.vertex > vert) {
                --newEdge.vertex;
            }
        }
        // Update the texture parameters.
    ...
    public boolean[] mergeEdges(boolean[] selected) {
        ...
        for (int i = 0; i < edges.length; ++i) {
            ...
            if (selected[esel]) {
                ...
                while (edges[e].face == face1 && edges[edges[e].hedge].face == face2 && selected[esel] && e != i) {
                    ++count;
                    e = edges[e].next;
                    esel = e;
                    if (esel >= edges.length / 2) {
        ...
        for (int i = 1; i < selected.length; ++i) {
            for (int l = 0; l < i; ++l) {
                ...
                if (selected[i] && selected[l]) {
                    ...
                    boolean merge = false;
                    for (int j = 0; j < edges.length / 2; ++j) {
                        if ((edges[j].face == l && edges[edges[j].hedge].face == i) || (edges[j].face == i && edges[edges[j].hedge].face == l)) {
                            merge = true;
                            edgeSel[j] = true;
                        }
                    }
                ...
                if (fullSel[ed] && fullSel[ned]) {
                    ...
                    if ((prod > 1e-4) && (e.length > 2)) {
                        ...
                        if (Math.abs(n1.dot(n2.cross(n3))) < 1e-4) {
                            ...
                            cuts[ed] = t1.plus(t2).plus(t3).times(1.0 / n1.dot(n2.cross(n3)));
                        }
                    } else {
            ...
            for (int item : e) {
                ...
                if ((cutsTable[ed] != -1) && (cuts[ed] == null)) {
                    if (p3.distance(vertices[i].r) < 1e-6) {
                        log.info("#1");
                        p1 = vertices[edges[item].vertex].r.minus(vertices[i].r);
                        if (p1.length() < 1e-4) {
        ...
        for (int i = 0; i < edges.length / 2; ++i) {
            ...
            if (selected[i]) {
                ...
                if (ref1 != -1) {
                    ...
                    for (int k : ved) {
                        newEdges[newEdges[k].hedge].vertex = cutsTable[i];
                    }
                ...
                if (ref2 != -1) {
                    ...
                    for (int k : ved) {
                        newEdges[newEdges[k].hedge].vertex = cutsTable[ed2];
                    }
                ...
                if (ref1 == -1) {
                    newEdges[ec2] = new Wedge(cutsTable[i], hec2, faceCount, nhed);
                    newEdges[ec2].smoothness = newEdges[i].smoothness;
                    newSel[ec2] = true;
                    newEdges[hec2] = new Wedge(cutsTable[ed1], ec2, newEdges[newEdges[ned1].hedge].face, newEdges[ned1].hedge);
    ...
    private void removeSmallEdgeLengths() {
        ...
        for (int i = 0; i < edges.length; ++i) {
            ...
            Wedge edge = edges[i];
            Vec3 v = vertices[edge.vertex].r.minus(vertices[edges[edge.hedge].vertex].r);
            if (v.length() < edgeLengthLimit) {
                sel[i] = sel[edge.hedge] = true;
                remove = true;
            }
        }
        if (remove) {
    ...
    public void mirrorMesh() {
        ...
        if (changed) {
            ...
            for (int e : boundaries) {
                ...
                for (Wedge newEdge : newEdges) {
                    if (newEdge.vertex == v + vertices.length) {
                        newEdge.vertex = v;
                    }
                    if (newEdge.next == newEdges[newe].hedge) {
                        newEdge.next = e;
                    }
                }
            ...
            for (Wvertex wvertex : nv) {
                wvertex.edge = edgeTable[wvertex.edge];
            }

            for (Wedge wedge : ne) {
                wedge.vertex = vertTable[wedge.vertex];
                wedge.hedge = edgeTable[wedge.hedge];
                wedge.next = edgeTable[wedge.next];
            }
    ...
    public void invertNormals() {
        ...
        int next;
        for (Wface face : faces) {
            int[] fe = getFaceEdges(face);
            for (int j = 0; j < fe.length; j++) {
                int i = fe[j];
                newEdges[i] = new Wedge(edges[i]);
                newEdges[i].vertex = edges[edges[i].hedge].vertex;
                next = j - 1;
                if (next < 0) {
                    next = fe.length - 1;
                }
                newEdges[i].next = fe[next];
                if (edges[edges[i].hedge].face == -1) {
                    newEdges[newEdges[i].hedge] = new Wedge(edges[edges[i].hedge]);
                    newEdges[newEdges[i].hedge].vertex = edges[i].vertex;
                    int p = i;
                    while (edges[p].next != edges[i].hedge) {
                        p = edges[edges[p].next].hedge;
                    }
                    newEdges[newEdges[i].hedge].next = p;
                }
            }

        }
        for (Wvertex vertex : vertices) {
            vertex.edge = edges[vertex.edge].hedge;
        }
        edges = newEdges;
        resetMesh();
    }
    ...
    public boolean joinBoundaries(int one, int two) {
        ...
        for (int item : getVertexEdges(vertices[one])) {
            if (edges[item].face == -1) {
                ob = item;
            }
        }
        for (int value : getVertexEdges(vertices[two])) {
            if (edges[value].face == -1) {
                tb = value;
            }
        }
        if (ob == -1 || tb == -1) {
    ...
    private void blindConnectVertices(int one, int two) {
        ...
        for (int value : getVertexEdges(vertices[one])) {
            for (int k : te) {
                if (edges[value].face == edges[k].face) {
                    ob = value;
                    tb = k;
                }
            }
            if (edges[value].vertex == two) {
                return;
            }
        }
    ...
    public boolean[] findSimilarEdges(boolean[] selected, double tol) {
        ...
        for (int i = 0; i < selected.length; i++) {
            if (selected[i]) {
                length = vertices[edges[i].vertex].r.distance(vertices[edges[edges[i].hedge].vertex].r);
                for (int j = 0; j < selected.length; j++) {
                    diff = length
                            - vertices[edges[j].vertex].r.distance(vertices[edges[edges[j].hedge].vertex].r);
    ...
    public int[] openSeams() {
        ...
        List<Integer> vertTable = new ArrayList<>();
            ...
            for (int i = 0; i < edges.length / 2; i++) {
                if (seamsCopy[i]) {
                    ...
                    if (bv1 == -1 && bv2 == -1) {
                        ...
                        newEdges[nhe] = new Wedge(vertCount, edgeCount, newEdges[he].face, newEdges[he].next);
                        newVertices[vertCount] = new Wvertex(new Vec3(newVertices[v1].r), edgeCount);
                        ref = v1;
                        while (ref >= orVertsLength) {
                            ref = vertTable.get(ref - orVertsLength);
                        }
    ...
    public boolean addFaceFromPoints(int from, int to, Vec3[] newPoints) {
        ...
        while (fromPoint != to) {
            found = false;
            for (int j : getVertexEdges(vertices[fromPoint])) {
                if (edges[j].face < 0) {
                    found = true;
                    if (j < edges.length / 2) {
                        v1.add(j);
                    } else {
                        v1.add(j + newEdges.length / 2 - edges.length / 2);
                    }
                    fromPoint = edges[j].vertex;
                    break;
                }
            }
            if (fromPoint == from) {
            ...
            for (int j : getVertexEdges(vertices[fromPoint])) {
                if (edges[j].face < 0) {
                    ...
                    if (j < edges.length / 2) {
                        ...
                        v2.add(j + newEdges.length / 2 - edges.length / 2);
                    }
                    fromPoint = edges[j].vertex;
                    break;
                }
        ...
        for (int value : ve) {
            if (edges[edges[value].hedge].next == index) {
                index = edges[value].hedge;
                if (index >= edges.length / 2) {
                    index += newEdges.length / 2 - edges.length / 2;
                }
    ...
    public String checkMesh() {
        ...
        for (int i = 0; i < vertices.length; ++i) {
            if (edges[vertices[i].edge].hedge >= 0 && edges[vertices[i].edge].hedge < edges.length && edges[edges[vertices[i].edge].hedge].vertex != i) {
        ...
        if (validHedge & validVertex) {
            for (int i = 0; i < edges.length; ++i) {
                Wedge edge = edges[i];
                if (edge.hedge < edges.length / 2 && i < edges.length / 2) {
                    s.append("Wrong position in array for edges ").append(i).append(" and ").append(edge.hedge).append(".\n");
                }
                if (edge.hedge >= edges.length / 2 && i >= edges.length / 2) {
                    s.append("Wrong position in array for edges ").append(i).append(" and ").append(edge.hedge).append(".\n");
                }
                if (edge.face == -1 && edges[edge.hedge].face == -1) {
                    s.append("Edges ").append(i).append(" and other half-edge").append(edge.hedge).append(" both boundary edges.\n");
                }
                if (edge.next >= edges.length) {
                    edge.next = 0;
     ...
     *
     * @param vert Vertices involved
     * @param coef And coefficients
     * @author Francois Guillet
     * @created February, 12, 2005
     */
        public record VertexParamInfo(int[] vert, double[] coef) {
        }
