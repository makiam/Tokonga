...
public class VRMLExporter {
     ...
     * @param parent the parent ObjectInfo, if <code>info</code> is a child, otherwise <i>null</i>.
    ...
    private static void writeObject(ObjectInfo info, ObjectInfo parent, PrintWriter out, double tol, boolean smooth, int indent, Scene theScene, TextureImageExporter textureExporter) {
        ...
        if (obj instanceof Cube && !hasTexture) {
            ...
            if (mesh != null) {
                ...
                for (Vec3 vert1 : vert) {
                    pos[0] = Math.round(vert1.x * 1e6) / 1e6;
                    pos[1] = Math.round(vert1.y * 1e6) / 1e6;
                    pos[2] = Math.round(vert1.z * 1e6) / 1e6;
    ...
    private static void writeMesh(FacetedMesh mesh, ObjectInfo info, PrintWriter out, int indent, Scene theScene, TextureImageExporter textureExporter, boolean includeNormals) {
        ...
        for (MeshVertex vert1 : vert) {
            pos[0] = Math.round(vert1.r.x * 1e6) / 1e6;
            pos[1] = Math.round(vert1.r.y * 1e6) / 1e6;
            pos[2] = Math.round(vert1.r.z * 1e6) / 1e6;
        ...
        if (includeNormals) {
            ...
            for (Vec3 norm1 : norm) {
                if (norm1 == null) {
                    ...
                    pos[0] = Math.round(norm1.x * 1e6) / 1e6;
                    pos[1] = Math.round(norm1.y * 1e6) / 1e6;
                    pos[2] = Math.round(norm1.z * 1e6) / 1e6;
                    write(pos[0] + " " + pos[1] + " " + pos[2] + ",", out, indent + 3);
                }
            }
            write("] }", out, indent + 2);
        ...
        if (ti != null && ((Object3D) mesh).getTextureMapping() instanceof UVMapping && ((UVMapping) ((Object3D) mesh).getTextureMapping()).isPerFaceVertex(mesh)) {
            ...
            for (Vec2[] coord1 : coords) {
                for (Vec2 coord : coord1) {
     ...
     * <ul>
     * <li>if there is a char in the corresponding position in 'replace', then it replaces the char
     * (ie, the char is translated).
     * <li>if there is no corresponding char in 'replace' then nothing is copied (ie, the char is
     * deleted).
     * </ul>
     * <br>
     * <li>if the char is <em>not</em> in 'match' then
     * <ul>
     * <li>if 'replace' is longer than 'match' and the char matches one of the extra chars in
     * 'replace', then the char is copied.
     ...
     *
     * Examples: match = "({[+=-" replace = ")}]"
     *
     * will convert ( to ); { to }; [ to ]; delete all +=- and copy all else
     *
     * match = 'A' replace = '' copy all but A
     *
     * match = '' replace = 'A' delete all but A
     *
     * match = 'A' replace = 'A' copy all (including A)
     *
     * match = '' replace = '' delete all.
     *
     * If this all seems arbitrary, then the following explanation may help:
     *
     * Any char in 'match' with no corresponding char in 'replace' is specified for deletion.
     * Conversely, any char in 'replace' with no corresponding char in 'match' is specified for
     * copying.
     *
     * If 'match' is longer than 'replace', then this specifies conditional deletion (those chars in
     * 'match' with no corresponding char in 'replace'), and hence any chars not matched in 'match'
     * are copied transparently.
     *
     * So if 'replace' is empty, and 'match' is non empty, then the chars in 'match' are deleted,
     * and everything else is copied.
     *
     * Conversely, if 'replace' is longer than 'match', then this specifies conditional copying
     * (those chars in 'replace' with no corresponding char in 'match'), and hence any chars not
     * matched in 'match' and not in the extra chars in 'replace' are deleted.
     *
     * So if 'match' is empty, and 'replace' is non-empty, then 'replace' specifies chars to be
     * copied, and everything else is deleted.
     *
     * If 'match' and 'replace' are idenical non-null strings, then the effect is to copy
     * everything, without change. (each char in 'match' is translated to itself, and all others are
     * copied).
     *
     * If 'match' and 'replace' are both null strings, then the effect is to delete all characters.
     * (null 'match' means no translations, and null 'replace' means no copying).
     */
